...
class GriffinKernelProvisioner(LocalProvisioner):
    """
    :class:`GriffinKernelProvisioner` is a kernel provisioner that is used to provision
    griffin-kernels for the Griffin IDE.
    """
    ...
    async def pre_launch(self, **kwargs: Any) -> dict[str, Any]:
        ...
        # Replace the `ipykernel_launcher` with `griffin_kernel.console`
        cmd_indx = kernel_cmd.index("ipykernel_launcher")
        if cmd_indx != -1:
            kernel_cmd[cmd_indx] = "griffin_kernels.console"
