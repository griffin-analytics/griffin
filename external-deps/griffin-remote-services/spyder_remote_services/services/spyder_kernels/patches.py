...
from griffin_remote_services.services.griffin_kernels.provisioner import GriffinKernelProvisioner
...
class GriffinMainKernelHandler(MainKernelHandler):
    @web.authenticated
    @authorized
    async def post(self):
        """Start a kernel."""
        ...
        kernel_id = await ensure_async(
            km.start_kernel(  # type:ignore[has-type]
                ...
                griffin_kernel=model.get("griffin_kernel", False)
            )
        )
...
def __patch_async_start_kernel(func: Callable[P, T]):
    ...
    async def wrapper(self: KernelManager, *args: P.args, **kw: P.kwargs) -> T:
        ...
        if kw.pop("griffin_kernel", False):
            self.provisioner = GriffinKernelProvisioner(
...
def patch_maping_kernel_manager(obj: AsyncMappingKernelManager):
    ...
    obj.default_kernel_name = "griffin-kernel"
...
def patch_main_kernel_handler(router: Router):
    for idx, rule in enumerate(router.rules):
        if isinstance(rule.target, Router):
            ...
            router.rules[idx].target = GriffinMainKernelHandler
            break
