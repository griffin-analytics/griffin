...
# Install griffin-remote-services
...
# Install griffin-kernel
if [ $KERNEL_VERSION == "latest" ]; then
  micromamba create -y -n $KERNEL_ENV -c conda-forge -c conda-forge/label/griffin_kernels_rc "python=${PYTHON_VERSION}" griffin-kernels
elif [[ $KERNEL_VERSION != *"="* ]] && [[ $KERNEL_VERSION != *">="* ]] && [[ $KERNEL_VERSION != *"<="* ]] && [[ $KERNEL_VERSION != *">"* ]] && [[ $KERNEL_VERSION != *"<"* ]]; then
  micromamba create -y -n $KERNEL_ENV -c conda-forge -c conda-forge/label/griffin_kernels_rc "python=${PYTHON_VERSION}" "griffin-kernels=$KERNEL_VERSION"
else
  micromamba create -y -n $KERNEL_ENV -c conda-forge -c conda-forge/label/griffin_kernels_rc "python=${PYTHON_VERSION}" "griffin-kernels${KERNEL_VERSION}"
fi
