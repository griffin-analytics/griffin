...
"""
Class that handles communications between Griffin kernel and frontend.
...
The messages exchanged have the following msg_dict:
    ...
    ```
    msg_dict = {
        'griffin_msg_type': griffin_msg_type,
        'content': content,
    }
    ```
...
The messages exchanged are:
    - Function call (griffin_msg_type = 'remote_call'):
    ...
    - If the 'settings' has `'blocking' =  True`, a reply is sent.
      (griffin_msg_type = 'remote_call_reply'):
...
class CommBase:
    ...
    # ---- Private -----
    def _send_message(
        self, griffin_msg_type, content=None, comm_id=None, buffers=None
    ):
        """
        ...
        Parameters
        ----------
        griffin_msg_type: str
            The griffin message type
        content: dict
    ...
    def _comm_message(self, msg):
        """
        Handle internal griffin messages.
        """
        self.calling_comm_id = msg['content']['comm_id']

        # Get message dict
        msg_dict = msg['content']['data']
        griffin_msg_type = msg_dict['griffin_msg_type']
        buffers = msg['buffers']

        if griffin_msg_type in self._message_handlers:
            self._message_handlers[griffin_msg_type](msg_dict, buffers)
        else:
            logger.debug("No such griffin message type: %s" % griffin_msg_type)
    ...
    def _remote_callback(self, call_name, call_args, call_kwargs):
        ...
        raise CommError("No such griffin call type: %s" % call_name)
