...
"""
Griffin magics related to code execution, debugging, profiling, etc.
"""
...
# Local imports
from griffin_kernels.comms.frontendcomm import frontend_request
...
from griffin_kernels.customize.umr import UserModuleReloader
...
@magics_class
class GriffinCodeRunner(Magics):
    """
    ...
    def _exec_file(
        ...
        try:
            ...
            print(
                ...
                "while trying to get the file code from Griffin's  editor. "
                "The error was:\n\n"
            )
        ...
        with NamespaceManager(
            ...
            if "multiprocessing" in sys.modules:
                # See https://github.com/griffin-ide/griffin/issues/16696
                try:
            ...
            if wdir is not None:
                ...
                if os.path.isdir(wdir):
                    ...
                    # See https://github.com/griffin-ide/griffin/issues/13632
    ...
    def _exec_cell(
        ...
        """
        try:
            # Get code from griffin
            cell_code = frontend_request(blocking=True).run_cell(cell_id, filename)
        except Exception:
            print(
                "This command failed to be executed because an error occurred "
                "while trying to get the cell code from Griffin's editor."
                "The error was:\n\n"
            )
            self.shell.showtraceback(exception_only=True)
            return
    ...
    def _get_current_file_name(self):
        ...
        try:
            ...
            print(
                "This command failed to be executed because an error occurred "
                "while trying to get the current file name from Griffin's editor."
                "The error was:\n\n"
            )
            self.shell.showtraceback(exception_only=True)
            return None
        ...
        try:
            ...
            if capture_last_expression:
                ast_code, capture_last_expression = capture_last_Expr(
                    ast_code, "_griffin_out", ns_globals
                )
            ...
            if capture_last_expression:
                out = ns_globals.pop("_griffin_out", None)
                if out is not None:
                    return out
