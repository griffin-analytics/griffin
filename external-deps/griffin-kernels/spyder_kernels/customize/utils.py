    ...
    if (
        ...
        if sys.version_info[:2] >= (3, 8):
            ...
            if assign_node.lineno == assign_node.end_lineno:
                # Add '__griffin_builtins__.globals()[{}] = ' and remove 'None'
...
def exec_encapsulate_locals(
    ...
    * In general, the dict returned by locals() might or might not be modified.
    ...
    if use_locals_hack:
        globals["__griffin_builtins__"] = builtins
        ...
        # Create a function
        indent = "    "
        code = ["def _griffinpdb_code():"]

        # Add locals in globals
        # If the debugger is recursive, the globals could already
        # have a _griffinpdb_locals as it might be shared between
        # levels
        if "_griffinpdb_locals" in globals:
            globals["_griffinpdb_locals"].append(locals)
        else:
            globals["_griffinpdb_locals"] = [locals]

        # Load locals if they have a valid name
        # In comprehensions, locals could contain ".0" for example
        code += [indent + "{k} = _griffinpdb_locals[-1]['{k}']".format(
            k=k) for k in locals if k.isidentifier()]

        # The code comes here

        # Update the locals
        code += [indent + "_griffinpdb_locals[-1].update("
                 "__griffin_builtins__.locals())"]

        # Run the function
        code += ["_griffinpdb_code()"]
    ...
    try:
        ...
        if use_locals_hack:
            # Cleanup code
            globals.pop("_griffinpdb_code", None)
            if len(globals["_griffinpdb_locals"]) > 1:
                del globals["_griffinpdb_locals"][-1]
            else:
                del globals["_griffinpdb_locals"]
