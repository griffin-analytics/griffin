...
import griffin_kernels
from griffin_kernels.comms.commbase import stacksummary_to_json
from griffin_kernels.comms.frontendcomm import CommError, frontend_request
from griffin_kernels.customize.utils import (
...
class GriffinPdb(ipyPdb):
    ...
    def __init__(self, completekey='tab', stdin=None, stdout=None,
        ...
        super(GriffinPdb, self).__init__()
        ...
        # enable it in Griffin.
        # Fixes griffin-ide/griffin#20639.
            ...
            try:
                ...
                if line.rstrip()[-1:] == ";":
                    ...
                    code_ast, capture_last_expression = capture_last_Expr(
                        code_ast, "_griffinpdb_out", global_ns)
                ...
                if capture_last_expression:
                    out = global_ns.pop("_griffinpdb_out", None)
                    if out is not None:
                        sys.stdout.flush()
                        sys.stderr.flush()
                        try:
    ...
    def interaction(self, frame, traceback):
        ...
        with DebugWrapper(self):
            ...
            return super(GriffinPdb, self).interaction(
                frame, traceback)
    ...
    def stop_here(self, frame):
        ...
        if self.pdb_ignore_lib and path_is_library(filename):
            return False
        if self.skip_hidden and os.path.dirname(griffin_kernels.__file__) in filename:
            # This is griffin-kernels internals
            return False
        return True
    ...
    def should_continue(self, frame):
        ...
        Fixes griffin-ide/griffin#2034
        """
        ...
        if not frame:
            # We are not debugging, return. Solves griffin-ide/griffin#10290
            return False
        ...
        # Fixes griffin-ide/griffin#4681
    ...
    def do_where(self, arg):
        ...
        return super(GriffinPdb, self).do_where(arg)
    ...
    # --- Methods overriden by us for Griffin integration
    def postloop(self):
        # postloop() is called when the debugger’s input prompt exists. Reset
        # _previous_step so that get_pdb_state() actually notifies Griffin
    ...
    def recursive_debugger(self):
        ...
        try:
            ...
            # Reset _previous_step so that get_pdb_state() notifies Griffin about
    ...
    def user_return(self, frame, return_value):
        ...
        super(GriffinPdb, self).user_return(frame, return_value)
    ...
    def cmdloop(self, intro=None):
        ...
        while not stop:
            if self.cmdqueue:
                ...
                # Fixes griffin-ide/griffin#22500
                line = (
    ...
    # --- Methods defined by us for Griffin integration
    def set_griffin_breakpoints(self, breakpoints):
        """Set Griffin breakpoints."""
        ...
        for fname, data in list(breakpoints.items()):
            for linenumber, condition in data:
                try:
                    ...
                    # Fixes griffin/issues/15546
                    # The file is not readable
                    pass

    breakpoints = property(fset=set_griffin_breakpoints)
    ...
    def get_pdb_state(self):
        ...
        if self.pdb_publish_stack:
            # Publish Pdb stack so we can update the Debugger plugin on Griffin
    ...
    def run(self, cmd, globals=None, locals=None):
        ...
        """
        with DebugWrapper(self):
            super(GriffinPdb, self).run(cmd, globals, locals)
    ...
    def runeval(self, expr, globals=None, locals=None):
        ...
        """
        with DebugWrapper(self):
            super(GriffinPdb, self).runeval(expr, globals, locals)
    ...
    def runcall(self, *args, **kwds):
        ...
        """
        with DebugWrapper(self):
            super(GriffinPdb, self).runcall(*args, **kwds)

    def set_remote_filename(self, filename):
        """Set remote filename to signal Griffin on mainpyfile."""
