...
from griffin_kernels.comms.decorators import (
    register_comm_handlers, comm_handler)
from griffin_kernels.utils.pythonenv import (
    get_env_dir,
    is_conda_env,
    is_pyenv_env,
    PythonEnvInfo,
...
from griffin_kernels.console.shell import GriffinShell
from griffin_kernels.comms.utils import WriteContext
...
class GriffinKernel(IPythonKernel):
    """Griffin kernel for Jupyter."""

    shell_class = GriffinShell

    def __init__(self, *args, **kwargs):
        super(GriffinKernel, self).__init__(*args, **kwargs)
    ...
    def kernel_info(self):
        # Used for checking correct version by griffin
        infos = super().kernel_info
        infos.update({
            "griffin_kernels_info": (
                griffin_kernels.__version__,
                sys.executable
            )
        })
        return infos
    ...
    def enable_faulthandler(self):
        ...
        if sys.platform.startswith('linux'):
            ...
            try:
                ...
                fault_dir = os.path.join(xdg_data_home, "griffin")
    ...
    def _do_complete(self, code, cursor_pos):
        ...
        return super(GriffinKernel, self).do_complete(code, cursor_pos)
    ...
    def is_defined(self, obj, force_import=False):
        ...
        from griffin_kernels.utils.dochelpers import isdefined
    ...
    def get_doc(self, objtxt):
        ...
        try:
            ...
            pass
        from griffin_kernels.utils.dochelpers import getdoc
    ...
    def get_source(self, objtxt):
        """Get object source"""
        from griffin_kernels.utils.dochelpers import getsource
    ...
    def get_mpl_interactive_backend(self):
        ...
        """
        # Backends that Griffin can handle
    ...
    def set_configuration(self, conf):
        ...
        for key, value in conf.items():
            if key == "cwd":
                ...
                # Fixes griffin-ide/griffin#23484
    ...
    def set_color_scheme(self, color_scheme):
        self.shell.set_griffin_theme(color_scheme)
    ...
    def _is_list(self, var):
        ...
        # The try/except is necessary to fix griffin-ide/griffin#19516.
        try:
        ...
        try:
            ...
            if "GUI eventloops" in str(err):
                ...
                if backend not in previous_backend.lower():
                    ...
                    error = (
                        "\n"
                        "NOTE: Griffin *can't* set your selected Matplotlib "
    ...
    def _load_wurlitzer(self):
        ...
        if not os.name == 'nt':
            ...
            # Fixes griffin-ide/griffin#8668
            try:
    ...
    def _get_comm(self, comm_id):
        ...
        Fixes griffin-ide/griffin#15498
        """
        try:
