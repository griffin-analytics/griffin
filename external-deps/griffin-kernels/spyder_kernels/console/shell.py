...
"""
Griffin shell for Jupyter kernels.
"""
...
# Local imports
from griffin_kernels.customize.namespace_manager import NamespaceManager
from griffin_kernels.customize.griffinpdb import GriffinPdb
from griffin_kernels.customize.code_runner import GriffinCodeRunner
from griffin_kernels.comms.commbase import stacksummary_to_json
from griffin_kernels.comms.decorators import comm_handler
from griffin_kernels.utils.mpl import automatic_backend


logger = logging.getLogger(__name__)


class GriffinShell(ZMQInteractiveShell):
    """Griffin shell."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.special = None
        self._pdb_conf = {}
        super(GriffinShell, self).__init__(*args, **kwargs)
        self._allow_kbdint = False
        self.register_debugger_sigint()
        self.update_gui_frontend = False
        self._griffin_theme = 'dark'

        # Substrings of the directory where Griffin-kernels is installed
        self._package_locations = [
            # When the package is properly installed
            os.path.join("site-packages", "griffin_kernels"),
            # When it's installed from the external-deps subrepo. We need this
            # for our tests
            os.path.join("external-deps", "griffin-kernels", "griffin_kernels")
        ]
    ...
    def init_magics(self):
        """Init magics"""
        super().init_magics()
        self.register_magics(GriffinCodeRunner)
        ...
        if etype is bdb.BdbQuit:
            ...
            for line in stb:
                if (
                    ...
                    griffin_stb.append(line)

        super()._showtraceback(etype, evalue, griffin_stb)

    def set_griffin_theme(self, theme):
        """Set the theme for the console."""
        self._griffin_theme = theme
        if theme == "dark":
    ...
    def get_griffin_theme(self):
        """Get the theme for the console."""
        return self._griffin_theme
    ...
    def enable_matplotlib(self, gui=None):
        ...
        # Fixes griffin-ide/griffin#22420.
        if gui == "inline":
    ...
    def is_debugging(self):
        ...
        for session in self._namespace_stack[::-1]:
            if isinstance(session, GriffinPdb) and session.curframe is not None:
                return True
        return False
    ...
    def pdb_session(self):
        ...
        for session in self._namespace_stack[::-1]:
            if isinstance(session, GriffinPdb):
                return session
        return None
    ...
    def showtraceback(self, exc_tuple=None, filename=None, tb_offset=None,
        ...
        super(GriffinShell, self).showtraceback(
    ...
    def register_debugger_sigint(self):
        ...
        signal.signal(signal.SIGINT, self.griffinkernel_sigint_handler)
    ...
    def request_pdb_stop(self):
        ...
        elif (self.griffinkernel_sigint_handler
              == signal.getsignal(signal.SIGINT)):
            # Use griffinkernel_sigint_handler
    ...
    def griffinkernel_sigint_handler(self, signum, frame):
        """SIGINT handler."""
