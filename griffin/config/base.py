...
from griffin.utils import encoding

#==============================================================================
# Only for development
#==============================================================================
# To activate/deactivate certain things for development
# GRIFFIN_DEV is (and *only* has to be) set in bootstrap.py
DEV = os.environ.get('GRIFFIN_DEV')

# Manually override whether the dev configuration directory is used.
USE_DEV_CONFIG_DIR = os.environ.get('GRIFFIN_USE_DEV_CONFIG_DIR')
...
def get_safe_mode():
    """
    Make Griffin use a temp clean configuration directory for testing
    purposes GRIFFIN_SAFE_MODE can be set using the --safe-mode option.
    """
    return bool(os.environ.get('GRIFFIN_SAFE_MODE'))
...
def running_under_pytest():
    ...
    variable GRIFFIN_PYTEST is defined in conftest.py.
    """
    return bool(os.environ.get('GRIFFIN_PYTEST'))
...
def running_remoteclient_tests():
    ...
    This function is used to do some adjustment for testing. The environment
    variable GRIFFIN_TEST_REMOTE_CLIENT is 'true' in conftest.py.
    """
    return bool(os.environ.get("GRIFFIN_TEST_REMOTE_CLIENT") == "true")
...
def running_in_binder():
    ...
    return (
        ...
        and "griffin-ide/binder-environments" in os.environ["BINDER_REPO_URL"]
    )
...
def get_debug_level():
    debug_env = os.environ.get('GRIFFIN_DEBUG', '')
...
def get_conf_subfolder():
    ...
    # Griffin settings dir
    # NOTE: During the 2.x.x series this dir was named .griffin2, but
    # since 3.0+ we've reverted back to use .griffin to simplify major
    ...
    if osp.isdir(path):
        ...
        if not path:
            raise RuntimeError('Please set the environment variable HOME to '
                               'your user/home directory path so Griffin can '
                               'start properly.')
...
def get_custom_conf_dir():
    ...
    """
    custom_dir = os.environ.get('GRIFFIN_CONFDIR')
    if custom_dir:
        custom_dir = osp.abspath(custom_dir)

        # Set env var to not lose its value in future calls when the cwd
        # is changed by Griffin.
        os.environ['GRIFFIN_CONFDIR'] = custom_dir
        return custom_dir
...
def get_conf_path(filename=None):
    ...
    if running_under_pytest() or get_safe_mode():
        ...
        # on Linux, as it was requested on griffin-ide/griffin#2629.
...
def get_conf_paths():
    ...
    if os.name == 'nt':
        SEARCH_PATH = (
            'C:/ProgramData/griffin',
        )
    else:
        SEARCH_PATH = (
            '/etc/griffin',
            '/usr/local/etc/griffin',
        )

    if CONDA_PREFIX is not None:
        CONDA_PREFIX = CONDA_PREFIX.replace('\\', '/')
        SEARCH_PATH += (
            '{}/etc/griffin'.format(CONDA_PREFIX),
        )

    SEARCH_PATH += (
        '{}/etc/griffin'.format(sys.prefix),
    )
...
def is_py2exe_or_cx_Freeze():
    """Return True if this is a py2exe/cx_Freeze distribution of Griffin"""
    return osp.isfile(osp.join(get_module_path('griffin'), osp.pardir))
...
# This needs to be updated every time a new language is added to griffin, and is
...
def get_available_translations():
    """
    List available translations for griffin based on the folders found in the
    ...
    """
    locale_path = get_module_data_path("griffin", relpath="locale",
        ...
        if lang not in LANGUAGE_CODES:
            if DEV:
                error = ('Update LANGUAGE_CODES (inside config/base.py) if a '
                         'new translation has been added to Griffin. '
...
def get_interface_language():
    """
    If Griffin has a translation available for the locale language, it will
    return the version provided by Griffin adjusted for language subdifferences,
    otherwise it will return DEFAULT_LANGUAGE.

    Example:
    1.) Griffin provides ('en', 'de', 'fr', 'es' 'hu' and 'pt_BR'), if the
    locale is either 'en_US' or 'en' or 'en_UK', this function will return 'en'

    2.) Griffin provides ('en', 'de', 'fr', 'es' 'hu' and 'pt_BR'), if the
    locale is either 'pt' or 'pt_BR', this function will return 'pt_BR'
    """

    # Solves griffin-ide/griffin#3627.
    try:
    ...
    if locale_language is not None:
        griffin_languages = get_available_translations()
        for lang in griffin_languages:
...
def load_lang_conf():
    ...
    try to use the local settings if Griffin provides a translation, or
...
def get_translation(modname, dirname=None):
    ...
    if os.name == 'nt':
        # Trying to set LANG on Windows can fail when Griffin is
        # run with admin privileges.
        # Fixes griffin-ide/griffin#6886.
        try:
...
def is_conda_based_app(pyexec=sys.executable):
    """
    Check if Griffin is running from the conda-based installer by looking for
    the `griffin-menu.json` file.
...
SAVED_CONFIG_FILES = ('help', 'onlinehelp', 'path', 'pylint.results',
                      'griffin.ini', 'temp.py', 'temp.spydata', 'template.py',
                      'history.py', 'history_internal.py', 'workingdir',
                      '.projects', '.griffinproject', '.ropeproject',
                      'monitor.log', 'monitor_debug.log', 'rope.log',
                      'langconfig', 'griffin.lock',
                      'config{}griffin.ini'.format(os.sep),
...
def reset_config_files():
    ...
    print("*** Reset Griffin settings to defaults ***", file=STDERR)
