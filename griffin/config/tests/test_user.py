...
from griffin.config.user import NoDefault, UserConfig
from griffin.utils.fixtures import tmpconfig
...
class TestUserConfigGet:
    ...
    def test_userconfig_get_does_not_eval_functions(self, userconfig):
        # Regression test for griffin-ide/griffin#3354.
...
# --- GriffinUserConfig tests
# ============================================================================
# --- Compatibility API
class TestGriffinConfigApplyPatches:

    def test_griffinconfig_apply_configuration_patches_42(
            self, griffinconfig_patches_42):
        # Check that the value is updated
        value = griffinconfig_patches_42.get('ipython_console',
                                            'startup/run_lines')
        expected_value = 'value1; value2'
        assert value == expected_value

    def test_griffinconfig_apply_configuration_patches_45(
            self, griffinconfig_patches_45):
        # Check that the value is not updated
        value = griffinconfig_patches_45.get('ipython_console',
                                            'startup/run_lines')
        expected_value = 'value1,value2'
        assert value == expected_value


def test_griffinconfig_get_defaults_path_name_from_version(griffinconfig):
    func = griffinconfig.get_defaults_path_name_from_version
    _, name = func('50.0.0')
    assert name == 'defaults-50.0.0'

    path, name = func('51.0.0')
    assert name == 'defaults-griffin-test-51.0.0'
    assert path.endswith('defaults')

    path, name = func('53.0.0')
    assert name == 'defaults-griffin-test-53.0.0'
    assert path.endswith('defaults')
