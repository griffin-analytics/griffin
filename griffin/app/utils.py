...
"""Utility functions for the Griffin application."""
...
from griffin.utils.external.dafsa.dafsa import DAFSA
from griffin.utils.image_path_manager import get_image_path
from griffin.utils.installers import running_installer_test
from griffin.utils.palette import GriffinPalette
from griffin.utils.qthelpers import file_uri, qapplication

# For griffin-ide/griffin#7447.
try:
...
FILTER_NAMES = os.environ.get('GRIFFIN_FILTER_LOG', "").split(',')
...
class Spy:
    ...
    namespace to inspect Griffin internals.

    Attributes:
        app       Reference to main QApplication object
        window    Reference to griffin.MainWindow widget
    """
...
def set_opengl_implementation(option):
    """
    Set the OpenGL implementation used by Griffin.

    See griffin-ide/griffin#7447 for the details.
    """
...
def setup_logging(cli_options):
    ...
    if cli_options.debug_info or get_debug_level() > 0:
        ...
        filepath = os.environ['GRIFFIN_DEBUG_FILE']
        handlers.append(
...
def delete_debug_log_files():
    ...
    debug_file = os.environ['GRIFFIN_DEBUG_FILE']
...
def qt_message_handler(msg_type, msg_log_context, msg_string):
    ...
    if msg_string not in BLACKLIST:
        print(msg_string)  # griffin: test-skip
...
def create_splash_screen(use_previous_factor=False):
    ...
    use_previous_factor: bool, optional
        ...
        when restarting Griffin, so the screen looks as expected. Default is
        False.
    """
    if not running_under_pytest():
        ...
        # shown when Griffin is restarted. Otherwise, it appears pixelated.
...
def create_application():
    ...
    try:
        ...
        if running_under_pytest():
            ...
            from griffin.config.fonts import MEDIUM, MONOSPACE
    ...
    if hasattr(app, 'setDesktopFileName'):
        app.setDesktopFileName('griffin')
    ...
    def spy_excepthook(type_, value, tback):
        ...
        if running_installer_test():
            # This will exit Griffin with exit code 1 without invoking
...
def create_window(WindowClass, app, splash, options, args):
    """
    Create and show Griffin's main window and start QApplication event loop.
    ...
    options: argparse.Namespace
        Command line options passed to Griffin
    args: list
        List of file names passed to the Griffin executable in the
        command line.
    """
    # Main window
