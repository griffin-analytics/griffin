...
Developed and maintained by the Griffin Project
Contributors
...
#==============================================================================
from griffin import requirements
...
from qtpy.QtWidgets import (
    QApplication, QMainWindow, QMessageBox, QShortcut, QTabBar, QMenu)
...
try:
    #from qtpy.QtWebEngineWidgets import WEBENGINEnet stop fontcache
    from qtpy.QtWebEngineWidgets import WEBENGINE, stop, fontcache
    #del %windir%\System32\FNTCACHE.DAT
    os.remove(os.path.join(os.environ['windir'], 'System32', 'FNTCACHE.DAT'))
    #net start fontcache
    subprocess.run(['net', 'start', 'fontcache'], check=True)
except ImportError:
    WEBENGINE = False
...
from griffin.api.shortcuts import GriffinShortcutsMixin
from griffin.api.widgets.mixins import GriffinMainWindowMixin
from griffin.config.base import (_, DEV, get_conf_path, get_debug_level,
                                get_home_dir, is_conda_based_app,
                                running_under_pytest, STDERR)
from griffin.config.gui import is_dark_font_color
from griffin.config.main import OPEN_FILES_PORT
from griffin.config.manager import CONF
from griffin.config.utils import IMPORT_EXT
from griffin.py3compat import to_text_string
from griffin.utils import encoding, programs
from griffin.utils.icon_manager import ima
from griffin.utils.misc import select_port, getcwd_or_home
from griffin.utils.palette import GriffinPalette
from griffin.utils.qthelpers import file_uri, qapplication, start_file
from griffin.utils.stylesheet import APP_STYLESHEET

# Griffin API Imports
from griffin.api.exceptions import GriffinAPIError
from griffin.api.plugins import Plugins, GriffinDockablePlugin, GriffinPluginV2
from griffin.api.plugins._old_api import GriffinPlugin, GriffinPluginWidget
...
if os.name == 'nt':
    from griffin.utils.windows import (set_attached_console_visible,
...
# Main Window
#==============================================================================
class MainWindow(QMainWindow, GriffinMainWindowMixin, GriffinShortcutsMixin):
    """Griffin main window"""
    CONF_SECTION = 'main'
    ...
    def __init__(self, splash=None, options=None):
        ...
        if os.name == "nt":
            ...
            if not DEV:
                # Make griffin quit when presing ctrl+C in the console
                # In DEV Ctrl+C doesn't quit, because it helps to
                # capture the traceback when griffin freezes
        ...
        # attributes, i.e layout (Qt) vs layout (GriffinPluginV2)
    ...
    def get_plugin(self, plugin_name, error=True):
        ...
        if error:
            raise GriffinAPIError(f'Plugin "{plugin_name}" not found!')
    ...
    def get_dockable_plugins(self):
        ...
        for plugin_name in PLUGIN_REGISTRY:
            ...
            if isinstance(plugin, (GriffinDockablePlugin, GriffinPluginWidget)):
    ...
    def show_status_message(self, message, timeout):
        """
        Show a status message in Griffin Main Window.
        """
    ...
    def register_plugin(self, plugin_name, external=False, omit_conf=False):
        """
        Register a plugin in Griffin Main Window.
        """
        plugin = PLUGIN_REGISTRY.get_plugin(plugin_name)
        ...
        if isinstance(plugin, GriffinDockablePlugin):
    ...
    def unregister_plugin(self, plugin):
        """
        Unregister a plugin from the Griffin Main Window.
        """
    ...
    def _prevent_freeze_when_moving_dockwidgets(self):
        ...
        different locations (see `GriffinDockWidget` for how that's handled).
    ...
    def setup(self):
        ...
        from griffin.plugins.help.utils.sphinxify import CSS_PATH, DARK_CSS_PATH
        ...
        status.showMessage(_("Welcome to Griffin!"), 5000)
        ...
        for plugin in all_plugins.values():
            ...
            if plugin_name in internal_plugins:
                ...
                # See griffin-ide/griffin#17464 for more details.
        ...
        # Instantiate internal Griffin 4 plugins
        for plugin_name in internal_plugins:
            if plugin_name in enabled_plugins:
                PluginClass = internal_plugins[plugin_name]
                if issubclass(PluginClass, GriffinPlugin):
        ...
        # Instantiate external Griffin 5 plugins
    ...
    def pre_visible_setup(self):
        ...
        # Tabify new plugins which were installed or created after Griffin ran
        # for the first time.
    ...
    def post_visible_setup(self):
        ...
        # Fixes griffin-ide/griffin#12104
        ...
        # Fixes griffin-ide/griffin#22454.
        self.get_plugin(Plugins.Application).on_mainwindow_visible()
        QApplication.processEvents()

        # Server to maintain just one Griffin instance and open files in it if
        ...
        # Fixes griffin-ide/griffin#3887.
        self.menuBar().raise_()

        # Remove Help menu
        help_menu = self.menuBar().findChild(QMenu, "&Help")
        if help_menu:
            self.menuBar().removeAction(help_menu.menuAction())
    ...
    def restore_undocked_plugins(self):
        ...
        for plugin_name in PLUGIN_REGISTRY:
            ...
            elif isinstance(plugin, GriffinPluginWidget):
    ...
    def set_window_title(self):
        """Set window title."""
        if DEV is not None:
            title = u"Griffin %s (Python %s.%s)" % (__version__,
                                                   sys.version_info[0],
                                                   sys.version_info[1])
        elif is_conda_based_app():
            title = "Griffin"
        else:
            title = u"Griffin (Python %s.%s)" % (sys.version_info[0],
                                                sys.version_info[1])
    ...
    def closing(self, cancelable=False, close_immediately=False):
        ...
        if cancelable and self.get_conf('prompt_on_exit'):
            reply = QMessageBox.critical(self, 'Griffin',
        ...
        # Fixes griffin-ide/griffin#12139
    ...
    def open_file(self, fname, external=False):
        ...
        or open file outside Griffin (if extension is not supported)
        """
    ...
    def open_external_file(self, fname):
        ...
        variable explorer inside Griffin.
        """
        ...
        # Don't open script that starts Griffin at startup.
        # Fixes issue griffin-ide/griffin#14483
        if sys.platform == 'darwin' and 'bin/griffin' in fname:
            return
        ...
        if osp.isfile(fpath):
            ...
            QMessageBox.warning(
                self, _("Error"),
                _('To open <code>{fpath}</code> as a project with Griffin, '
                  'please use <code>griffin -p "{fname}"</code>.')
    ...
    def start_open_files_server(self):
        ...
        # Fixes griffin-ide/griffin#18262
        try:
        ...
        while 1:  # 1 is faster than True
            try:
                ...
                # See griffin-ide/griffin#1275 for details on why errno EINTR is
    ...
    def restart(self, reset=False, close_immediately=False):
        """Wrapper to handle plugins request to restart Griffin."""
...
def main(options, args):
    ...
    if options.reset_to_defaults:
        # Reset Griffin settings to defaults
        CONF.reset_to_defaults()
        return
    elif options.optimize:
        # Optimize the whole Griffin's source code directory
        import griffin
        programs.run_python_script(module="compileall",
                                   args=[griffin.__path__[0]], p_args=['-O'])
        return
    ...
    try:
        ...
        fonts_message = QMessageBox(
            ...
            "Griffin",
            _("It was not possible to load Griffin's icon theme, so Griffin "
              ...
              "supported by Griffin or Microsoft, or your system administrator "
