...
# Local imports
from griffin import __trouble_url__
from griffin.api.utils import get_class_values
from griffin.api.widgets.auxiliary_widgets import GriffinWindowWidget
from griffin.api.plugins import Plugins
from griffin.app.tests.conftest import (
...
from griffin.dependencies import DEPENDENCIES
from griffin.plugins.debugger.api import DebuggerWidgetActions
from griffin.plugins.externalterminal.api import ExtTerminalShConfiguration
from griffin.plugins.help.widgets import ObjectComboBox
from griffin.plugins.help.tests.test_plugin import check_text
from griffin.plugins.ipythonconsole.utils.kernel_handler import KernelHandler
from griffin.plugins.ipythonconsole.api import (
    IPythonConsolePyConfiguration,
    IPythonConsoleWidgetMenus
)
from griffin.plugins.mainmenu.api import ApplicationMenus
from griffin.plugins.layout.layouts import DefaultLayouts
from griffin.plugins.toolbar.api import ApplicationToolbars
from griffin.plugins.run.api import (
...
from griffin.plugins.shortcuts.widgets.table import SEQUENCE
from griffin.py3compat import qbytearray_to_str, to_text_string
from griffin.utils.conda import get_list_conda_envs
from griffin.utils.misc import remove_backslashes, rename_file
from griffin.utils.clipboard_helper import CLIPBOARD_HELPER
from griffin.utils.programs import find_program
from griffin.widgets.dock import DockTitleBar
...
def test_single_instance_and_edit_magic(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(
        lambda: shell.griffin_kernel_ready and shell._prompt_html is not None,
        timeout=SHELL_TIMEOUT)

    spy_dir = osp.dirname(get_module_path('griffin'))
    lock_code = (
        "import sys\n"
        "sys.path.append(r'{spy_dir_str}')\n"
        "from griffin.utils.external import lockfile\n"
        "lock_file = r'{lock_file}'\n"
        "lock = lockfile.FilesystemLock(lock_file)\n"
        "lock_created = lock.lock()\n"
        "print(lock_created)".format(
            spy_dir_str=spy_dir,
            lock_file=get_conf_path('griffin.lock'))
    )
...
def test_default_plugin_actions(main_window, qtbot):
    ...
    assert isinstance(main_widget.windowwidget, GriffinWindowWidget)
...
@pytest.mark.known_leak  # Opens Griffin/QtWebEngine/Default/Cookies
...
def test_get_help_ipython_console(main_window, qtbot):
    ...
    # Check that a expected text is part of the page
    qtbot.waitUntil(lambda: check_text(webpage, "GriffinShell"), timeout=6000)
...
def test_connection_to_external_kernel(main_window, qtbot):
    """Test that only Griffin kernels are connected to the Variable Explorer."""
    ...
    # Test with a kernel from Griffin
...
def test_project_path(main_window, tmpdir, qtbot):
    """Test project path added to griffin_pythonpath and IPython Console."""
    projects = main_window.projects

    # Create a project path
    path = str(tmpdir.mkdir('project_path'))
    assert path not in projects.get_conf(
        'griffin_pythonpath', section='pythonpath_manager')

    # Ensure project path is added to griffin_pythonpath
    projects.open_project(path=path)
    ...
    # Ensure that project path is removed from griffin_pythonpath
    assert path not in projects.get_conf(
        'griffin_pythonpath', section='pythonpath_manager')
...
def test_run_code(main_window, qtbot, tmpdir):
    ...
    # This is a regression for griffin-ide/griffin#23076
    with qtbot.waitSignal(shell.executed):
        re_run_action.trigger()
...
def test_open_files_in_new_editor_window(main_window, qtbot):
    ...
    Test for griffin-ide/griffin#4085.
    """
...
def test_close_when_file_is_changed(main_window, qtbot):
    """Test closing griffin when there is a file with modifications open."""
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
        lambda: shell.griffin_kernel_ready and shell._prompt_html is not None,
...
def test_plots_plugin(main_window, qtbot, tmpdir, mocker):
    ...
    mocker.patch('griffin.plugins.plots.widgets.figurebrowser.getsavefilename',
...
def test_plot_from_collectioneditor(main_window, qtbot):
    ...
    from griffin.widgets.collectionseditor import CollectionsEditor
...
def test_editorstack_open_switcher_dlg(main_window, tmpdir, qtbot):
    ...
    Regression test for griffin-ide/griffin#10684
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_run_static_code_analysis(main_window, qtbot):
    ...
    from griffin.plugins.pylint.main_widget import PylintWidgetActions
...
def test_troubleshooting_menu_item_and_url(main_window, qtbot, monkeypatch):
    ...
    attr_to_patch = ('griffin.utils.qthelpers.QDesktopServices')
...
def test_help_opens_when_show_tutorial_full(main_window, qtbot):
    """
    Test fix for griffin-ide/griffin#6317.
                ...
                if area['visible']:
                    for plugin_id in area['plugin_ids']:
                        if plugin_id not in area['hidden_plugin_ids']:
                            ...
                            print(plugin)  # griffin: test-skip
                            try:
                                # New API
...
def test_programmatic_custom_layouts(main_window, qtbot):
    ...
    mw.get_plugin('griffin_boilerplate')
...
def test_preferences_run_section_exists(main_window, qtbot):
    """
    Test for griffin-ide/griffin#13524 regression.
...
def test_preferences_checkboxes_not_checked_regression(main_window, qtbot):
    """
    Test for griffin-ide/griffin/#10139 regression.
...
def test_preferences_change_font_regression(main_window, qtbot):
    """
    Test for griffin-ide/griffin#10284 regression.
...
def test_preferences_empty_shortcut_regression(main_window, qtbot):
    """
    Test for griffin-ide/griffin/#12992 regression.
...
def test_preferences_shortcut_reset_regression(main_window, qtbot):
    """
    Test for griffin-ide/griffin/#11132 regression.
...
def test_varexp_rename(main_window, qtbot, tmpdir):
    ...
    Regression test for griffin-ide/griffin#10735
    """
    # ---- Setup ----
...
def test_varexp_remove(main_window, qtbot, tmpdir):
    ...
    Regression test for griffin-ide/griffin#10709
    """
    # ---- Setup ----
...
def test_pdb_key_leak(main_window, qtbot, tmpdir):
    """
    Check that pdb notify griffin doesn't call
...
def test_pdb_step(main_window, qtbot, tmpdir, where):
    """
    Check that pdb notify Griffin only moves when a new line is reached.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_debug_namespace(main_window, qtbot, tmpdir):
    ...
    Regression test for griffin-ide/griffin-kernels#394.
    """
    code1 = (
...
def test_ordering_lsp_requests_at_startup(main_window, qtbot):
    ...
    This is a regression test for griffin-ide/griffin#13351.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_pdb_ipykernel(main_window, qtbot):
    """Check if pdb works without griffin kernel."""
    # Test with a generic kernel
    km, kc = start_new_kernel()
...
def test_print_comms(main_window, qtbot):
    ...
    # Create some output from griffin call
    with qtbot.waitSignal(shell.executed):
        shell.execute(code)
    ...
    assert ("Output from griffin call" in control.toPlainText())
...
def test_add_external_plugins_to_dependencies(main_window, qtbot):
    ...
    assert 'griffin-boilerplate' in external_names
...
def test_shortcuts_in_external_plugins(main_window, qtbot):
    ...
    # Show plugin
    main_widget = main_window.get_plugin('griffin_boilerplate').get_widget()
...
def test_visible_plugins(main_window, qtbot):
    ...
    main_window.layouts.quick_layout_switch(DefaultLayouts.GriffinLayout)
    qtbot.wait(200)
...
def test_outline_namespace_package(main_window, qtbot, tmpdir):
    ...
    This is a regression test for issue griffin-ide/griffin#16406.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_switch_to_plugin(main_window, qtbot):
    ...
    This is a regression test for issue griffin-ide/griffin#19374.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_PYTHONPATH_in_consoles(main_window, qtbot, tmp_path):
    ...
    # Check that the inital configured griffin_pythonpath is empty
    assert ppm.get_container()._griffin_pythonpath == []
...
def test_clickable_ipython_tracebacks(main_window, qtbot, tmp_path):
    ...
    This is a regression test for issue griffin-ide/griffin#20407.
    """
    shell = main_window.ipyconsole.get_current_shellwidget()
    control = shell._control
    qtbot.waitUntil(lambda: shell._prompt_html is not None,
    ...
    # See griffin-ide/griffin#21186 for the details
...
def test_icons_in_menus(main_window, qtbot):
    ...
    #   default on Mac (see the definition of GriffinAction).
...
def test_undock_plugin_and_close(main_window, qtbot):
    ...
    This checks the functionality added in PR griffin-ide/griffin#19784.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_outline_in_maximized_editor(main_window, qtbot):
    ...
    This is a regression test for issue griffin-ide/griffin#16265.
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_custom_run_config_for_multiple_executors(
    ...
    This is a regression test for issue griffin-ide/griffin#22496
    """
    # Auxiliary function
...
def test_debug_file_with_modules_in_same_dir(main_window, qtbot, tmp_path):
    ...
    This is a regression test for issue griffin-ide/griffin#23694
    """
    # Wait until the window is fully up
    shell = main_window.ipyconsole.get_current_shellwidget()
    control = shell._control
...
def test_custom_run_config_with_cwd(main_window, qtbot, tmp_path):
    ...
    This is a regression test for issue griffin-ide/griffin#23866
    """
    # Auxiliary functions
