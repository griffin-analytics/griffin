...
# Fixes griffin-ide/griffin#15768
...
# Fixes griffin-ide/griffin#15163
root_logger = logging.getLogger()
root_logger.setLevel(logging.ERROR)

# Prevent a race condition with ZMQ
# See griffin-ide/griffin#5324.
import zmq

# Load GL library to prevent segmentation faults on some Linux systems
# See griffin-ide/griffin#3226 and griffin-ide/griffin#3332.
try:
...
from griffin.config.base import (get_conf_path, reset_config_files,
                                running_under_pytest, is_conda_based_app)
from griffin.utils.conda import get_conda_root_prefix
from griffin.utils.external import lockfile
from griffin.py3compat import is_text_string
...
# Do not rely on CONDA_PYTHON_EXE or CONDA_PREFIX in case Griffin is started
...
# this path to PATH. See griffin-ide/griffin#22374
...
if CLI_OPTIONS.conf_dir:
    os.environ['GRIFFIN_CONFDIR'] = CLI_OPTIONS.conf_dir
...
def send_args_to_griffin(args):
    """
    Simple socket client used to send the args passed to the Griffin
    ...
    for __ in range(200):
        try:
            ...
            # Print informative warning to let users know what Griffin is doing
            if print_warning:
...
def main():
    """
    Start Griffin application.

    If single instance mode is turned on (default behavior) and an instance of
    Griffin is already running, this will just parse and send command line
    ...
    os.environ['GRIFFIN_ARGS'] = str(sys.argv[1:])
    ...
    if sys.platform == 'darwin':
        # Fixes launching issues with Big Sur (griffin-ide/griffin#14222)
        os.environ['QT_MAC_WANTS_LAYER'] = '1'
        # Prevent Griffin from crashing in macOS if locale is not defined
        ...
        # Don't show useless warning in the terminal where Griffin
        # was started.
        # See griffin-ide/griffin#3730.
        os.environ['EVENT_NOKQUEUE'] = '1'
    else:
        # Prevent our kernels to crash when Python fails to identify
        # the system locale.
        # Fixes griffin-ide/griffin#7051.
        try:
            from locale import getlocale
            getlocale()
        except ValueError:
            # This can fail on Windows. See griffin-ide/griffin#6886.
            try:
                os.environ['LANG'] = 'C'
                os.environ['LC_ALL'] = 'C'
            except Exception:
    ...
    if options.debug_info:
        ...
        os.environ['GRIFFIN_DEBUG'] = levels[options.debug_info]

    _filename = 'griffin-debug.log'
    if options.debug_output == 'file':
        _filepath = osp.realpath(_filename)
    else:
        _filepath = get_conf_path(_filename)
    os.environ['GRIFFIN_DEBUG_FILE'] = _filepath

    if options.paths:
        from griffin.config.base import get_conf_paths
        sys.stdout.write('\nconfig:' + '\n')
    ...
    if (CONF.get('main', 'single_instance') and not options.new_instance
        ...
        # Try to lock griffin.lock. If it's *possible* to do it, then
        ...
        try:
            ...
            # module, try to remove a possibly stale griffin.lock.
            # This is reported to solve all problems with lockfile.
            # See griffin-ide/griffin#2363.
            try:
                if os.name == 'nt':
            ...
            # Then start Griffin as usual and *don't* continue
            # executing this script because it doesn't make
            # sense
            from griffin.app import mainwindow
            if running_under_pytest():
        ...
        if lock_created:
            ...
            # Pass args to Griffin or print an informative
            # message
            if args:
                send_args_to_griffin(args)
            else:
                print("Griffin is already running. If you want to open a new \n"
