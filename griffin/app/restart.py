...
A helper script that allows to restart (and also reset) Griffin from within the
running application.
"""
...
# Local imports
from griffin.app.utils import create_splash_screen
from griffin.config.base import _
from griffin.utils.image_path_manager import get_image_path
from griffin.utils.encoding import to_unicode
...
CLOSE_ERROR, RESET_ERROR, RESTART_ERROR = [1, 2, 3]  # Griffin error codes
...
class Restarter(QWidget):
    ...
    def launch_error_message(self, error_type, error=None):
        ...
        error_type : int [CLOSE_ERROR, RESET_ERROR, RESTART_ERROR]
            Possible error codes when restarting/resetting griffin.
        error : Exception
            Actual Python exception error caught.
        """
        messages = {CLOSE_ERROR: _("It was not possible to close the previous "
                                   "Griffin instance.\nRestart aborted."),
                    RESET_ERROR: _("Griffin could not reset to factory "
                                   "defaults.\nRestart aborted."),
                    RESTART_ERROR: _("It was not possible to restart Griffin.\n"
                                     "Operation aborted.")}
        titles = {CLOSE_ERROR: _("Griffin exit error"),
                  RESET_ERROR: _("Griffin reset error"),
                  RESTART_ERROR: _("Griffin restart error")}
...
def main():
    ...
    APP_ICON = QIcon(get_image_path("griffin"))
    app.setWindowIcon(APP_ICON)
    restarter.set_splash_message(_('Closing Griffin'))

    # Get variables
    griffin_args = env.pop('GRIFFIN_ARGS', None)
    pid = env.pop('GRIFFIN_PID', None)
    is_bootstrap = env.pop('GRIFFIN_IS_BOOTSTRAP', None)
    reset = env.pop('GRIFFIN_RESET', 'False')

    # Get the griffin base folder based on this file
    griffin_dir = osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))

    if not any([griffin_args, pid, is_bootstrap, reset]):
        error = "This script can only be called from within a Griffin instance"
    ...
    args = ast.literal_eval(griffin_args)
    reset = ast.literal_eval(reset)

    # GRIFFIN_DEBUG takes presedence over GRIFFIN_ARGS
    if '--debug' in args:
        args.remove('--debug')
    ...
    # Enforce the --new-instance flag when running griffin
    ...
    if is_bootstrap:
        script = osp.join(griffin_dir, 'bootstrap.py')
    else:
        script = osp.join(griffin_dir, 'griffin', 'app', 'start.py')
    ...
    # Before launching a new Griffin instance we need to make sure that the
    ...
    for counter in range(int(wait_time / SLEEP_TIME)):
        ...
        # The old griffin instance took too long to close and restart aborts
        restarter.launch_error_message(error_type=CLOSE_ERROR)

    # Reset Griffin (if required)
    # -------------------------------------------------------------------------
    if reset:
        restarter.set_splash_message(_('Resetting Griffin to defaults'))
