...
# Local imports
from griffin.config.base import _, running_in_ci, is_conda_based_app
...
SPHINX_REQVER = '>=0.6.6'
GRIFFIN_KERNELS_REQVER = '>=3.0.0,<3.2.0'
...
DESCRIPTIONS = [
    ...
    {'modname': "applaunchservices",
     ...
     'features': _("Notify macOS that Griffin can open Python files"),
    ...
    {'modname': 'pyls_griffin',
     'package_name': 'pyls-griffin',
     'features': _('Griffin plugin for the Python LSP Server'),
     'required_version': PYLS_GRIFFIN_REQVER},
    {'modname': 'pyuca',
     'package_name': 'pyuca',
    ...
    {'modname': "griffin_kernels",
     'package_name': "griffin-kernels",
     'features': _("Jupyter kernels for the Griffin console"),
     'required_version': GRIFFIN_KERNELS_REQVER},
     {'modname': "superqt",
...
class Dependency(object):
    ...
    def check(self):
        ...
        if self.modname == 'griffin_kernels':
            # TODO: Remove when griffin-kernels 3 is released!
            return True
...
def add(modname, package_name, features, required_version,
        installed_version=None, kind=MANDATORY):
    """Add Griffin dependency"""
    global DEPENDENCIES
    for dependency in DEPENDENCIES:
        # Avoid showing an unnecessary error when running our tests.
        if running_in_ci() and 'griffin_boilerplate' in modname:
            continue
