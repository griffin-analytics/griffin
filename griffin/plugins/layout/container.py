...
from griffin.plugins.layout.api import BaseGridLayoutType
from griffin.plugins.layout.layouts import DefaultLayouts
from griffin.plugins.layout.widgets.dialog import (
...
class LayoutContainer(PluginMainContainer):
    """
    Plugin container class that handles the Griffin quick layouts functionality.
    """

    def setup(self):
        # Basic attributes to handle layouts options and dialogs references
        self.griffin_layouts = OrderedDict()
        ...
        self._reset_action = self.create_action(
            ...
            text=_('Reset to Griffin default'),
    ...
    def update_layout_menu_actions(self):
        ...
        for name in order:
            if name in active:
                if name in self.griffin_layouts:
                    index = name
                    name = self.griffin_layouts[index].get_name()
                else:
    ...
    def get_layout(self, layout_id):
        ...
        Instance of a griffin.plugins.layout.api.BaseGridLayoutType subclass
            Layout.
        """
        if layout_id not in self.griffin_layouts:
            raise GriffinAPIError(
                "Layout with id `{}` is not registered!".format(layout_id)
            )

        return self.griffin_layouts[layout_id]
    ...
    def show_save_layout(self):
        ...
        active = self.get_conf('active')
        dialog_names = [
            name for name in names if name not in self.griffin_layouts.keys()
        ]
    ...
    def show_layout_settings(self):
        ...
        order = self.get_conf('order')
        active = self.get_conf('active')
        read_only = list(self.griffin_layouts.keys())
    ...
    def toggle_layout(self, direction='next'):
        ...
        if current_layout is None:
            # Start from default
            current_layout = names.index(DefaultLayouts.GriffinLayout)
        ...
        is_layout_id = names[index_or_layout_id] in self.griffin_layouts
