...
from griffin.api.plugins import (
    Plugins, DockablePlugins, GriffinDockablePlugin, GriffinPluginV2)
...
from griffin.plugins.mainmenu.api import ApplicationMenus, ViewMenuSections
from griffin.plugins.layout.container import (
...
from griffin.plugins.layout.layouts import (DefaultLayouts,
                                           ...
                                           GriffinLayout, VerticalSplitLayout)
from griffin.plugins.preferences.api import PreferencesActions
from griffin.plugins.toolbar.api import (
    ApplicationToolbars, MainToolbarSections)
from griffin.py3compat import qbytearray_to_str  # FIXME:
...
# ----------------------------------------------------------------------------
# This defines the layout version used by different Griffin releases. In case
...
#
# * Griffin 4: Version 0 (it was the default).
# * Griffin 5.0.0: Version 1 (a bump was required due to the new API).
# * Griffin 5.1.0: Version 2 (a bump was required due to the migration of
#                            Projects to the new API).
# * Griffin 5.2.0: Version 3 (a bump was required due to the migration of the
#                            IPython Console to the new API)
# * Griffin 6.0.0: Version 4 (a bump was required due to the migration of
#                            Editor to the new API)

WINDOW_STATE_VERSION = 4


class Layout(GriffinPluginV2, GriffinShortcutsMixin):
    """
    ...
    def on_initialize(self):
        ...
        self.register_layout(self, GriffinLayout)
    ...
    def before_mainwindow_visible(self):
        ...
        # https://github.com/griffin-ide/griffin/pull/22232#issuecomment-2224142496
    ...
    def on_mainwindow_visible(self):
        ...
        # Fixes griffin-ide/griffin#17945 and griffin-ide/griffin#21596
        self.setup_layout(default=False)
    ...
    def register_layout(self, parent_plugin, layout_type):
        ...
        Parameters
        ----------
        parent_plugin: griffin.api.plugins.GriffinPluginV2
            Plugin registering the layout type.
        layout_type: griffin.plugins.layout.api.BaseGridLayoutType
            Layout to register.
        """
    ...
    def get_layout(self, layout_id):
        ...
        Returns
        -------
        Instance of a griffin.plugins.layout.api.BaseGridLayoutType subclass
            Layout.
        """
    ...
    def setup_layout(self, default=False):
        ...
        self._first_griffin_run = False
        if hexstate is None:
            # First Griffin execution:
            self.main.setWindowState(Qt.WindowMaximized)
            self._first_griffin_run = True
            self.setup_default_layouts(DefaultLayouts.GriffinLayout, settings)
            ...
            if default:
                order = list(self.get_container().griffin_layouts.keys())
                ...
                ui_names = [
                    l.get_name()
                    for l in self.get_container().griffin_layouts.values()
                ]
            ...
            # Store the initial layout as the default in griffin
            prefix = 'layout_default/'
            self.save_current_window_settings(prefix, section, none_state=True)
            self._current_quick_layout = DefaultLayouts.GriffinLayout
    ...
    def setup_default_layouts(self, layout_id, settings):
        ...
        first_griffin_run = bool(self._first_griffin_run)  # Store copy

        if first_griffin_run:
            self.set_window_settings(*settings)
        else:
    ...
    def quick_layout_switch(self, index_or_layout_id):
        ...
        try:
            ...
            if hexstate is None:
                ...
                # defaults).  See griffin-ide/griffin#6202.
                if index_or_layout_id not in DEFAULT_LAYOUTS:
                    container.critical_message(
                        _("Warning"),
                        _("Error opening the custom layout.  Please close"
                          " Griffin and try again.  If the issue persists,"
                          " then you must use 'Reset to Griffin default' "
    ...
    def load_window_settings(self, prefix, default=False, section='main'):
        ...
        *prefix*, under *section*.
        ...
        # with the current screen. See griffin-ide/griffin#3748.
        width = pos[0]
        height = pos[1]
    ...
    def set_window_settings(self, hexstate, window_size, pos, is_maximized,
        ...
        if hexstate:
            ...
            # Check layout validity. Griffin 4 and below use the version 0
            # state (default), whereas Griffin 5 will use version 1 state.
    ...
    def save_current_window_settings(self, prefix, section='main',
        ...
        # Fixes griffin-ide/griffin#13882
        ...
        self.set_conf(
            ...
            # Fixes griffin-ide/griffin#20851
    ...
    def create_plugins_menu(self):
        ...
        for plugin in self.get_dockable_plugins():
            ...
            if action:
                ...
                # Fixes griffin-ide/griffin#21074
        ...
        # https://github.com/griffin-ide/griffin/issues/22189#issuecomment-2248644546
    ...
    def restore_visible_plugins(self):
        ...
        for plugin in visible_plugins:
            ...
            if (
                plugin_class
                # This check is necessary for griffin-ide/griffin#21074
    ...
    def tabify_plugin(self, plugin, default=None):
        ...
        if plugin.get_conf('first_time', True):
            ...
            # default layout.
            if (
                isinstance(plugin, GriffinDockablePlugin)
