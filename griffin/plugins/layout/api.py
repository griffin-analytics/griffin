...
class BaseGridLayoutType:
    """
    A base layout type to create custom layouts for Griffin panes.
    ...
    def _check_layout_validity(self):
        ...
        # Check ID
        if self.ID is None:
            raise GriffinAPIError("A Layout must define an `ID` class "
                                 "attribute!")
        ...
        if not self._areas:
            raise GriffinAPIError("A Layout must define add least one area!")
        ...
        if not any(self._visible_areas):
            raise GriffinAPIError("At least 1 area must be `visible`")

        # Check that there is a `default` area!
        if not any(default_areas):
            raise GriffinAPIError("No area is the `default`!")

        # Check that there is 1 `default` area!
        if default_areas.count(True) != 1:
            raise GriffinAPIError("Only 1 area can be the `default`!")
    ...
    def add_area(
        ...
        if self._default_added and default:
            raise GriffinAPIError("A default location has already been "
                                 "defined!")
    ...
    def set_main_window_layout(self, main_window, dockable_plugins):
        ...
        for plugin in dockable_plugins:
            ...
            # Fixes griffin-ide/griffin#22494
            ...
            # Fixes griffin-ide/griffin#22584
