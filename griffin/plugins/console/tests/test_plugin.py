...
from griffin.api.plugins import GriffinPluginV2
...
# Auxiliary classes
# =============================================================================
class MyPlugin(GriffinPluginV2):
    NAME = 'my-plugin'
...
def test_handle_exception(console_plugin, mocker):
    ...
    mocker.patch('griffin.widgets.reporterror.GriffinErrorDialog.show',
                 return_value=None)

    # --- Test internal errors in Griffin
...
def test_report_external_repo(console_plugin, mocker):
    ...
    mocker.patch.object(GriffinErrorDialog, 'set_github_repo_org')
    ...
    with pytest.raises(GriffinAPIError) as excinfo:
    ...
    error_data['repo'] = 'griffin-ide/griffin'
    with pytest.raises(GriffinAPIError) as excinfo:
        widget.handle_exception(error_data, sender=my_plugin)
    ...
    # Assert we called the method that sets the repo in GriffinErrorDialog
    call_args = GriffinErrorDialog.set_github_repo_org.call_args
    ...
    call_args = GriffinErrorDialog.set_github_repo_org.call_args
    assert call_args == call('griffin-ide/griffin')
