...
from griffin_kernels.utils.dochelpers import (getargtxt, getdoc, getobjdir,
                                             getsource)

# Local imports
from griffin import get_versions
from griffin.api.translations import _
from griffin.plugins.console.utils.interpreter import Interpreter
from griffin.py3compat import to_binary_string, to_text_string
...
from griffin.plugins.console.widgets.shell import PythonShellWidget
from griffin.plugins.variableexplorer.widgets.objecteditor import oedit
...
class SysOutput(QObject):
    ...
    # We need to add this method to fix griffin-ide/griffin#1789.
    def flush(self):
        pass

    # This is needed to fix griffin-ide/griffin#2984.
    @property
    def closed(self):
        return False
...
class InternalShell(PythonShellWidget):
    ...
    def __init__(self, parent=None, commands=[], message=None,
        ...
        # 'open_in_griffin' function in builtins)
        if hasattr(builtins, 'open_in_griffin'):
            self.sig_go_to_error_requested.connect(
                self.open_with_external_griffin)
    ...
    #------ External editing
    def open_with_external_griffin(self, text):
        """Load file in external Griffin's editor, if available
        ...
        if match:
            ...
            builtins.open_in_griffin(fname, int(lnb))
        ...
        if not self.multithreaded:
            if 'input' not in cmd:
                ...
                self.write(_('In order to use commands like "raw_input" '
                             'or "input" run Griffin with the multithread '
