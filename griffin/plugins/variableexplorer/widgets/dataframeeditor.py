...
# Copyright (c) 2014- Griffin Project Contributors
#
...
from griffin_kernels.utils.lazymodules import numpy as np, pandas as pd
...
from griffin.plugins.variableexplorer.widgets.arrayeditor import get_idx_rect
from griffin.plugins.variableexplorer.widgets.basedialog import BaseDialog
from griffin.plugins.variableexplorer.widgets.preferences import (
    PreferencesDialog
)
from griffin.py3compat import (is_text_string, is_type_text_string,
                              to_text_string)
from griffin.utils.icon_manager import ima
from griffin.utils.palette import GriffinPalette
from griffin.utils.qthelpers import keybinding, qapplication
...
BACKGROUND_NONNUMBER_COLOR = GriffinPalette.COLOR_BACKGROUND_2
...
class DataFrameModel(QAbstractTableModel, GriffinFontsMixin):
    """
    ...
    def max_min_col_update(self):
        ...
        for __, col in self.df.items():
            ...
            # Fixes griffin-ide/griffin#17145
            try:
                if (
    ...
    def get_bgcolor(self, index):
        ...
        if self.max_min_col[column] is None or pd.isna(value):
            ...
            # Fixes griffin-ide/griffin#18005
            try:
                if vmax - vmin == 0:
                    vmax_vmin_diff = 1.0
                else:
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            ...
            return self.get_font(GriffinFontType.MonospaceInterface)
...
class DataFrameView(QTableView, GriffinWidgetMixin):
    """
...
class DataFrameHeaderModel(QAbstractTableModel, GriffinFontsMixin):
    """
...
class DataFrameLevelModel(QAbstractTableModel, GriffinFontsMixin):
    """
    ...
    def __init__(self, model):
        super().__init__()
        self.model = model
        self._background = QColor(GriffinPalette.COLOR_BACKGROUND_2)
    ...
    def data(self, index, role):
        ...
        if not index.isValid():
            return None
        if role == Qt.FontRole:
            return self.get_font(GriffinFontType.Interface)
        label = ''
...
class DataFrameEditor(BaseDialog, GriffinWidgetMixin):
    """
    ...
    def setModel(self, model, relayout=True):
        ...
        # Fixes issue griffin-ide/griffin#20960
        self._reset_model(
            self.table_index,
    ...
    def show_preferences_dialog(self) -> None:
        ...
        if result == QDialog.Accepted:
            ...
            # Fixes griffin-ide/griffin#22629
            if float_format == 'd':
                float_format = '.0f'
