...
# Local imports
from griffin.plugins.variableexplorer.widgets.namespacebrowser import (
    NamespaceBrowser)
from griffin.widgets.collectionseditor import ROWS_TO_LOAD
from griffin.widgets.tests.test_collectioneditor import data, data_table
...
def test_sort_by_column(namespacebrowser, qtbot):
    ...
    Regression test for griffin-ide/griffin#9835 .
    """
    browser = namespacebrowser
...
def test_keys_sorted_and_sort_with_large_rows(namespacebrowser, qtbot):
    ...
    This is a regression test for issue griffin-ide/griffin#10702
    """
    browser = namespacebrowser
...
def test_namespacebrowser_plot_with_mute_inline_plotting_true(
    ...
    with patch('griffin.pyplot.subplots',
...
def test_namespacebrowser_plot_options(namespacebrowser):
    ...
    set to the values from the Griffin preferences when plotting.
    """
    def check_rc(*args):
...
def test_namespacebrowser_plot_with_mute_inline_plotting_false(namespacebrowser):
    ...
    with patch('griffin.pyplot.figure') as mock_figure, \
         patch('griffin.pyplot.plot') as mock_plot, \
         patch('griffin.pyplot.show') as mock_show:
        namespacebrowser.plot(my_list, 'plot')
