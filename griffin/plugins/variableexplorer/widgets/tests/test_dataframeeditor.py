...
from griffin.utils.test import close_message_box
from griffin.plugins.variableexplorer.widgets import dataframeeditor
...
def test_dataframe_to_type(qtbot):
    """Regression test for griffin-ide/griffin#12296"""
    # Setup editor
    d = {'col1': [1, 2], 'col2': [3, 4]}
    df = DataFrame(data=d)
...
def test_dataframe_editor_shows_scrollbar(qtbot):
    ...
    Regression test for griffin-ide/griffin#21627 .
    """
...
def test_dataframe_datetimeindex(qtbot):
    """Regression test for griffin-ide/griffin#11129 ."""
    ds = Series(
        numpy.arange(10),
...
def test_dataframemodel_sort_is_stable():   # cf. griffin-ide/griffin#3010.
...
def test_dataframemodel_with_timezone_aware_timestamps():
    # cf. griffin-ide/griffin#2940.
    df = DataFrame([x] for x in date_range('20150101', periods=5, tz='UTC'))
    dfm = DataFrameModel(df)
    assert dfm.max_min_col == [None]


def test_dataframemodel_with_categories():  # cf. griffin-ide/griffin#3308.
...
def test_dataframemodel_get_bgcolor_with_nullable_numbers():
    ...
    Regression test for griffin-ide/griffin#21222.
    """
    vals = [1, 2, 3, 4, 5]
...
def test_dataframemodel_with_format_percent_d_and_nan():
    ...
    Regression test for griffin-ide/griffin#4139.
    """
...
def test_dataframeeditor_refresh_after_edit(result):
    ...
    with patch('griffin.plugins.variableexplorer.widgets.dataframeeditor'
...
def test_change_format(qtbot):
    ...
    with patch(
        'griffin.plugins.variableexplorer.widgets.dataframeeditor'
...
def test_dataframemodel_with_format_thousands():
    ...
    Regression test for griffin-ide/griffin#14518.
    """
...
def test_dataframeeditor_menu_options(qtbot, monkeypatch):
    ...
    attr_to_patch = ('griffin.plugins.variableexplorer.widgets' +
    ...
    monkeypatch.setattr(
        QInputDialog,
        "getText",
        lambda *args: ("GRIFFINTEST_H", True)
    )
    header = editor.table_header.model()
...
def test_dataframeeditor_with_OutOfBoundsDatetime():
    # Test for griffin-ide/griffin#6177.
...
def test_dataframemodel_set_data_complex(monkeypatch):
    ...
    Unit regression test for griffin-ide/griffin#6115.
    """
    MockQMessageBox = Mock()
    attr_to_patch = ('griffin.plugins.variableexplorer.widgets' +
                     '.dataframeeditor.QMessageBox')
...
def test_dataframeeditor_edit_complex(qtbot, monkeypatch):
    ...
    Integration regression test for griffin-ide/griffin#6115.
    """
    MockQMessageBox = Mock()
    attr_to_patch = ('griffin.plugins.variableexplorer.widgets' +
                     '.dataframeeditor.QMessageBox')
