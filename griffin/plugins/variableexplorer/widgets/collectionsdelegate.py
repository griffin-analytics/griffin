...
from griffin_kernels.utils.nsview import (display_to_value, is_editable_type,
                                         is_known_type)

# Local imports
from griffin.api.fonts import GriffinFontsMixin, GriffinFontType
from griffin.config.base import _, is_conda_based_app
from griffin.py3compat import is_binary_string, is_text_string, to_text_string
from griffin.plugins.variableexplorer.widgets.arrayeditor import ArrayEditor
from griffin.plugins.variableexplorer.widgets.dataframeeditor import (
    DataFrameEditor)
from griffin.plugins.variableexplorer.widgets.texteditor import TextEditor
...
class CollectionsDelegate(QItemDelegate, GriffinFontsMixin):
    ...
    def createEditor(self, parent, option, index, object_explorer=False):
        ...
        try:
            ...
            if module in ['pandas', 'numpy']:
                ...
                message = _("Griffin is unable to show the {val_type} object "
                            "you're trying to view because <tt>{module}</tt> "
                            "is missing. Please install that package in your "
                            "Griffin environment to fix this problem.")
                ...
                if is_conda_based_app():
                    message = _("Griffin is unable to show the variable you're"
                                " trying to view because the module "
                                "<tt>{module}</tt> is not supported "
                                "by Griffin's standalone application.<br>")
                else:
                    message = _("Griffin is unable to show the variable you're"
                                " trying to view because the module "
                                "<tt>{module}</tt> is not found in your "
                                "Griffin environment. Please install this "
            ...
            QMessageBox.critical(
                self.parent(), _("Error"),
                _("Griffin was unable to retrieve the value of "
        ...
        # Fixes griffin-ide/griffin#10603
        ...
        # ArrayEditor for an images
            ...
            # Fixes griffin-ide/griffin#16731
            from .arrayeditor import ArrayEditor
            arr = np.array(value)
        ...
        # QLineEdit for an individual value (int, float, short string, etc)
            ...
            if readonly:
                ...
                # This is making Griffin crash because the QLineEdit that it's
        ...
        # ObjectExplorer for an arbitrary Python object
            ...
            from griffin.plugins.variableexplorer.widgets.objectexplorer \
    ...
    def commitAndCloseEditor(self):
        ...
        # but at least Griffin won't crash. It seems generated by a bug in sip.
        try:
    ...
    def paint(self, painter, option, index):
        ...
        if (
            ...
            # because in Griffin 6 is not intuitive how to do that since we use
            # a single click to open the editor associated to the cell.
            # Fixes griffin-ide/griffin#22524
