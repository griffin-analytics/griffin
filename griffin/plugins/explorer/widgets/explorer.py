...
from griffin.config.base import get_home_dir
from griffin.config.main import NAME_FILTERS
from griffin.plugins.explorer.widgets.utils import (
    create_script, fixpath, IconProvider, show_in_external_file_explorer)
from griffin.py3compat import to_binary_string
from griffin.utils import encoding
from griffin.utils.icon_manager import ima
from griffin.utils import misc, programs, vcs
from griffin.utils.misc import getcwd_or_home
from griffin.utils.qthelpers import (
...
class DirViewActions:
    ...
    OpenWithGriffin = 'open_with_griffin_action'
        ...
        if isinstance(option, QStyleOptionViewItem):
            ...
            if is_dir:
                ...
                if dir_path == self._project_dir:
                    option.icon = ima.icon("project_griffin")
...
# ---- Widgets
# ----------------------------------------------------------------------------
class DirView(QTreeView, GriffinWidgetMixin):
    ...
    """
    This signal is emitted to request opening a new file with Griffin.
    ...
    def setup(self):
        ...
        # Open actions
        self.open_with_griffin_action = self.create_action(
            DirViewActions.OpenWithGriffin,
            text=_("Open in Griffin"),
        ...
        self.vcs_commit_action = self.create_action(
            ...
            # Fixes griffin-ide/griffin#21959
            text="Git commit",
    ...
    def update_actions(self):
        ...
        self.open_with_griffin_action.setVisible(only_files and only_valid)
    ...
    def viewportEvent(self, event):
        ...
        # is being built. See griffin-ide/griffin#1250.
        #
    ...
    def edit_filter(self):
        ...
        # Dialog buttons
        button_box = GriffinDialogButtonBox(
    ...
    def open(self, fnames=None):
        ...
        for fname in fnames:
            if osp.isfile(fname) and encoding.is_text_file(fname):
                ...
                self.open_outside_griffin([fname])
    ...
    def open_outside_griffin(self, fnames):
        """
        Open file outside Griffin with the appropriate application.

        If this does not work, opening unknown file in Griffin, as text file.
        """
    ...
    def delete(self, fnames=None):
        ...
        yes_to_all = None
        for fname in fnames:
            griffinproject_path = osp.join(fname, '.griffinproject')
            if osp.isdir(fname) and osp.exists(griffinproject_path):
    ...
    def restore_directory_state(self, fname):
        ...
        if not osp.exists(root):
            # Directory has been (re)moved outside Griffin
            return
