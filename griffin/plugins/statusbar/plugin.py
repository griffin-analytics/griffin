...
from griffin.plugins.statusbar.confpage import StatusBarConfigPage
from griffin.plugins.statusbar.container import StatusBarContainer
...
class StatusBar(GriffinPluginV2):
    """Status bar plugin."""
    ...
    def add_status_widget(self, widget, position=StatusBarWidgetPosition.Left):
        ...
        if id_ in self.STATUS_WIDGETS and not running_under_pytest():
            raise GriffinAPIError(f'Status widget `{id_}` already added!')
    ...
    def get_status_widget(self, id_):
        ...
        if id_ in self.STATUS_WIDGETS:
            ...
            raise GriffinAPIError(f'Status widget "{id_}" not found!')
