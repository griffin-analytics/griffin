...
from griffin.plugins.pythonpath.utils import get_system_pythonpath
from griffin.plugins.pythonpath.widgets.pathmanager import PathManager
...
class PythonpathContainer(PluginMainContainer):
    ...
    def setup(self):
        ...
        self._griffin_pythonpath = self.get_conf('griffin_pythonpath')
    ...
    def update_active_project_path(self, path):
        ...
        self._project_path = OrderedDict()
        if path is None:
            logger.debug("Update Griffin PYTHONPATH because project was closed")
        else:
            logger.debug(f"Add project paths to Griffin PYTHONPATH: {path}")
    ...
    def get_griffin_pythonpath(self):
        """Return active Griffin PYTHONPATH as a list of paths."""
    ...
    def _save_paths(self, user_paths=None, system_paths=None, prioritize=None):
        ...
        on condition that _griffin_pythonpath changed.
        ...
        griffin_pythonpath = self.get_griffin_pythonpath()
        if griffin_pythonpath != self._griffin_pythonpath:
            logger.debug(f"Saving Griffin pythonpath: {griffin_pythonpath}")
            self.set_conf('griffin_pythonpath', griffin_pythonpath)
            self._griffin_pythonpath = griffin_pythonpath
            emit = True

        # Only emit signal if griffin_pythonpath or prioritize changed
        if emit:
            self.sig_pythonpath_changed.emit(
                self._griffin_pythonpath, self._prioritize
            )
