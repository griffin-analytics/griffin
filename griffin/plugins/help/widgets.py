...
from griffin.plugins.help.utils.sphinxify import (CSS_PATH, generate_context,
                                                 loading, usage, warning)
from griffin.plugins.help.utils.sphinxthread import SphinxThread
...
from griffin.utils.qthelpers import start_file
from griffin.widgets.comboboxes import EditableComboBox
    ...
    def validate(self, qstr, editing=True):
        ...
        if self.hasFocus() and valid is not None:
            if editing and not valid:
                ...
                if valid:
                    self.selected()
                    # See griffin-ide/griffin#9542.
                    self.lineEdit().cursorWordForward(False)
                else:
                    self.valid.emit(False, False)


class RichText(QWidget, GriffinWidgetMixin):
    """
...
class HelpWidget(PluginMainWidget):
    ...
    def __init__(self, name=None, plugin=None, parent=None):
        ...
        self.source_combo = GriffinComboBox(self)
    ...
    def show_intro_message(self):
        ...
        if self.get_conf('rich_mode'):
            title = _("Usage")
            tutorial_message = _("New to Griffin? Read our")
            tutorial = _("tutorial")
    ...
    def show_tutorial(self):
        """Show the Griffin tutorial."""
        tutorial_path = get_module_source_path('griffin.plugins.help.utils')
    ...
    def set_object_text(self, text, force_refresh=False, ignore_unknown=False):
        ...
        See Also
        --------
        :py:meth:griffin.widgets.mixins.GetHelpMixin.show_object_info
        """
    ...
    def set_internal_console(self, console):
        ...
        Parameters
        ----------
        console: :py:class:griffin.plugins.console.plugin.Console
            Console plugin.
        """
