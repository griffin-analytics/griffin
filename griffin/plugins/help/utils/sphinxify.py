...
# Copyright (C) 2010- Griffin Project Contributors
#
...
* The Griffin Project Contributors: Several changes to make it work with Griffin.
...
# Local imports
from griffin.config.base import (_, get_module_data_path,
...
# version of Griffin (i.e. the py2exe or cx_Freeze build)
CONFDIR_PATH = get_module_source_path('griffin.plugins.help.utils')
...
# use their own packages for them. See griffin-ide/griffin#1230, comment #7.
MATHJAX_PATH = get_module_data_path('griffin',
                                    relpath=osp.join('utils', 'help',
                                                     JS_PATH, 'mathjax'),
                                    attr_name='MATHJAXPATH')

JQUERY_PATH = get_module_data_path('griffin',
                                   relpath=osp.join('utils', 'help',
                                                    JS_PATH),
...
def sphinxify(docstring, context, buildername='html'):
    ...
    confdir = osp.join(get_module_source_path('griffin.plugins.help.utils'))
    srcdir = mkdtemp()
...
def generate_configuration(directory):
    ...
    conf = osp.join(get_module_source_path('griffin.plugins.help.utils'),
                    'conf.py')
