...
# Local imports
from griffin.api.asyncdispatcher import AsyncDispatcher
...
from griffin.plugins.ipythonconsole.api import IPythonConsoleWidgetActions
from griffin.plugins.mainmenu.api import (
    ApplicationMenus,
    ConsolesMenuSections,
    ToolsMenuSections,
)
from griffin.plugins.remoteclient.api import (
    RemoteClientActions,
    RemoteClientMenus,
)
from griffin.plugins.remoteclient.api import GriffinRemoteAPIManager
from griffin.plugins.remoteclient.api.protocol import (
    SSHClientOptions,
    ConnectionStatus,
)
from griffin.plugins.remoteclient.api.modules.file_services import (
    GriffinRemoteFileServicesAPI,
)
from griffin.plugins.remoteclient.widgets.container import RemoteClientContainer
...
class RemoteClient(GriffinPluginV2):
    """
    Remote client plugin.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._remote_clients: dict[str, GriffinRemoteAPIManager] = {}
    ...
    def load_client(self, config_id: str, options: SSHClientOptions):
        """Load remote server."""
        client = GriffinRemoteAPIManager(config_id, options, _plugin=self)
    ...
    @staticmethod
    def register_api(kclass: typing.Type[GriffinBaseJupyterAPIType]):
        """
        ...
        Parameters
        ----------
        kclass: Type[GriffinBaseJupyterAPI]
            Class to be registered.

        Returns
        -------
        Type[GriffinBaseJupyterAPI]
            Class that was registered.
        """
        return GriffinRemoteAPIManager.register_api(kclass)

    def get_api(
        self, config_id: str, api: str | typing.Type[GriffinBaseJupyterAPIType]
    ):
        """
        ...
        api: str | Type[GriffinBaseJupyterAPI]
            API class to be retrieved.

        Returns
        -------
        GriffinBaseJupyterAPI
            API class instance.
        """
        if config_id not in self._remote_clients:
            self.load_client_from_id(config_id)
    ...
    def get_file_api(self, config_id):
        ...
        return client.get_api(GriffinRemoteFileServicesAPI)
