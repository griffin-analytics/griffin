...
"""
griffin.plugins.remoteclient.api.manager
=======================================
...
from griffin.config.base import get_debug_level
from griffin.plugins.remoteclient import (
    GRIFFIN_REMOTE_MAX_VERSION,
    GRIFFIN_REMOTE_MIN_VERSION,
)
from griffin.plugins.remoteclient.api.modules.base import JupyterAPI
from griffin.plugins.remoteclient.api.protocol import (
    ConnectionInfo,
    ConnectionStatus,
...
from griffin.plugins.remoteclient.api.ssh import GriffinSSHClient
from griffin.plugins.remoteclient.utils.installation import (
...
if typing.TYPE_CHECKING:
    from griffin.plugins.remoteclient.api.modules.base import (
        GriffinBaseJupyterAPIType,
    )


class GriffinRemoteAPILoggerHandler(logging.Handler):
...
class GriffinRemoteAPIManager:
    ...
    START_SERVER_COMMAND = (
        f"/${{HOME}}/.local/bin/micromamba run -n {SERVER_ENV} griffin-server"
    )
    GET_SERVER_INFO_COMMAND = (
        f"/${{HOME}}/.local/bin/micromamba run"
        f" -n {SERVER_ENV} griffin-server info"
    )
    ...
    def __init__(self, conf_id, options: SSHClientOptions, _plugin=None):
        ...
        if self._plugin is not None:
            self.logger.addHandler(GriffinRemoteAPILoggerHandler(self))
    ...
    def server_url(self):
        ...
        is used to connect to the griffin-remote-server.
    ...
    def client_factory(self):
        ...
        return lambda: GriffinSSHClient(self)
    ...
    async def ensure_server_installed(self) -> bool:
        ...
        if Version(version) >= Version(GRIFFIN_REMOTE_MAX_VERSION):
            self.logger.error(
                f"Server version mismatch: {version} is greater than "
                f"the maximum supported version {GRIFFIN_REMOTE_MAX_VERSION}"
            )
        ...
        if Version(version) < Version(GRIFFIN_REMOTE_MIN_VERSION):
            self.logger.warning(
                f"Server version mismatch: {version} is lower than "
                f"the minimum supported version {GRIFFIN_REMOTE_MIN_VERSION}. "
    ...
    async def __install_remote_server(self):
        ...
        self.logger.debug(
            f"Installing griffin-remote-server on {self.peer_host}"
        )
        ...
        try:
            ...
            self.logger.error(
                f"Cannot install griffin-remote-server on "
        ...
        self.logger.info(
            f"Successfully installed griffin-remote-server on {self.peer_host}"
        )
    ...
    @classmethod
    def register_api(
        cls, kclass: type[GriffinBaseJupyterAPIType]
    ) -> type[GriffinBaseJupyterAPIType]:
        """Register a REST API class."""
        cls.REGISTERED_MODULE_APIS[kclass.__qualname__] = kclass
        return kclass

    def get_api(
        self, api: str | type[GriffinBaseJupyterAPIType]
    ) -> typing.Callable[..., GriffinBaseJupyterAPIType]:
