...
"""Tests for the Griffin Remote Client plugin."""
...
from griffin.plugins.remoteclient.tests.conftest import await_future
...
# =============================================================================
class TestNewServer:
    """Test the installation of the Griffin Remote Client plugin."""
...
class TestVersionCheck:
    def test_wrong_version(
        ...
        monkeypatch.setattr(
            "griffin.plugins.remoteclient.api.manager.GRIFFIN_REMOTE_MAX_VERSION",
            "0.0.1",
        )
        monkeypatch.setattr(
            "griffin.plugins.remoteclient.widgets.container.GRIFFIN_REMOTE_MAX_VERSION",
            "0.0.1",
        )

        def mock_critical(parent, title, text, buttons):
            assert "griffin-remote-services" in text
            assert "0.0.1" in text
            assert "is newer than" in text
            return QMessageBox.Ok

        monkeypatch.setattr(
            "griffin.plugins.remoteclient.widgets.container.QMessageBox.critical",
            mock_critical,
        )
