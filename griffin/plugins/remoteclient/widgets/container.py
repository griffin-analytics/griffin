...
from griffin.plugins.remoteclient import GRIFFIN_REMOTE_MAX_VERSION
...
from griffin.plugins.remoteclient.api.protocol import ConnectionInfo
from griffin.plugins.remoteclient.widgets.connectiondialog import (
    ConnectionDialog,
)
...
class RemoteClientContainer(PluginMainContainer):
    ...
    def on_server_version_mismatch(self, config_id, version: str):
        ...
        griffin-remote-services version installed in the server and the one
        ...
        QMessageBox.critical(
            ...
            _(
                "The version of <tt>griffin-remote-services</tt> on the "
                "remote host <b>{server}</b> (<b>{srs_version}</b>) is newer "
                "than the latest Griffin supports (<b>{max_version}</b>)."
                "<br><br>"
                "Please update Griffin to be able to connect to this host."
            ).format(
                server=server_name,
                srs_version=version,
                max_version=GRIFFIN_REMOTE_MAX_VERSION,
            ),
            QMessageBox.Ok,
        )
