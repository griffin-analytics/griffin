...
# TODO: Try to separate this module from griffin to create a self
...
from griffin_kernels.utils.dochelpers import getobj

# Local imports
from griffin.api.plugins import Plugins
from griffin.config.base import _, running_under_pytest
from griffin.plugins.editor.api.decoration import TextDecoration
from griffin.plugins.editor.api.panel import Panel
from griffin.plugins.editor.extensions import (CloseBracketsExtension,
                                              CloseQuotesExtension,
                                              DocstringWriterExtension,
                                              QMenuOnlyForEnter,
                                              EditorExtensionsManager,
                                              SnippetsExtension)
from griffin.plugins.editor.panels import (
    ClassFunctionDropdown, EdgeLine, FoldingPanel, IndentationGuide,
    LineNumberArea, PanelsManager, ScrollFlagArea)
from griffin.plugins.editor.utils.editor import (TextHelper, BlockUserData,
                                                get_file_language)
from griffin.plugins.editor.utils.kill_ring import QtKillRing
from griffin.plugins.editor.utils.languages import ALL_LANGUAGES, CELL_LANGUAGES
from griffin.plugins.editor.widgets.gotoline import GoToLineDialog
from griffin.plugins.editor.widgets.base import TextEditBaseWidget
from griffin.plugins.editor.widgets.codeeditor.lsp_mixin import LSPMixin
from griffin.plugins.editor.widgets.codeeditor.multicursor_mixin import (
    MultiCursorMixin
)
from griffin.plugins.outlineexplorer.api import (OutlineExplorerData as OED,
                                                is_cell_header)
from griffin.py3compat import to_text_string, is_string
...
from griffin.utils.qthelpers import file_uri, mimedata2url, start_file
from griffin.utils.vcs import get_git_remotes, remote_to_url
from griffin.utils.qstringhelpers import qstring_length
from griffin.widgets.mixins import HINT_MAX_WIDTH
...
class CodeEditor(LSPMixin, TextEditBaseWidget, MultiCursorMixin):
    ...
    def __init__(self, parent=None):
        ...
        # Linux Ubuntu. See griffin-ide/griffin#5215.
        self.setVerticalScrollBar(QScrollBar())

        # Highlights and flag colors
        self.warning_color = GriffinPalette.COLOR_WARN_2
        self.error_color = GriffinPalette.COLOR_ERROR_1
        self.todo_color = GriffinPalette.GROUP_9
        self.breakpoint_color = GriffinPalette.ICON_3
        self.occurrence_color = QColor(GriffinPalette.GROUP_2).lighter(160)
        self.found_results_color = QColor(GriffinPalette.COLOR_OCCURRENCE_4)
    ...
    def panels(self):
        ...
        :class:`griffin.widgets.panels.managers.PanelsManager`
    ...
    def setup_editor(self,
        ...
        if cloned_from is not None:
            ...
            # Fixes griffin-ide/griffin#23622
    ...
    def trim_trailing_newlines(self):
        ...
        # Fixes griffin-ide/griffin#16401
    ...
    def add_bookmark(self, slot_num, line=None, column=None):
        ...
        if line is None:
            # Triggered by shortcut, else by griffin start
            line, column = self.get_cursor_line_column()
    ...
    def paste(self):
        ...
        if clipboard.mimeData().hasUrls():
            ...
            # See griffin-ide/griffin#8644 for details.
            ...
            if all([url.isLocalFile() for url in urls]):
                if len(urls) > 1:
                    ...
                    # before proceeding.
                    # Fixes griffin-ide/griffin#17521
                    if urls:
    ...
    def __blockcomment_bar(self, compatibility=False):
        ...
        # Blockcomment bar in Griffin version >= 4
    ...
    def unblockcomment(self):
        ...
        # Needed for backward compatibility with Griffin previous blockcomments.
        # See griffin-ide/griffin#2845.
    ...
    def _handle_keypress_event(self, event):
        ...
        if text:
            ...
            # See griffin-ide/griffin#12663 and
    ...
    def keyPressEvent(self, event):
        ...
        if key in (Qt.Key_Enter, Qt.Key_Return):
            ...
            # See griffin-ide/griffin#495: on MacOS X, it is necessary to
    ...
    def _handle_goto_uri_event(self, pos):
        ...
        if key and pattern_text and cursor:
            ...
            if key in ['file']:
                ...
                if not osp.isfile(fname):
                    color = QColor(GriffinPalette.COLOR_ERROR_2)
...
class TestWidget(QSplitter):
    def __init__(self, parent):
        ...
        self.setWindowIcon(ima.icon('griffin'))
