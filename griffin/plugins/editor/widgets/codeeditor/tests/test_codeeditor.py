...
from griffin.widgets.mixins import TIP_PARAMETER_HIGHLIGHT_COLOR
...
def test_brace_match(codeeditor):
     ...
     * griffin-ide/griffin#2965
     * griffin-ide/griffin#9179
     * griffin-ide/griffin#14374

    If this test fails the best way to investigate is probably to open
    assets/braces.py in Griffin, step through the file and visually
    observe brace matching.

    Some caveats for brace matching can be found in pull request
    griffin-ide/griffin#14376
...
def test_editor_backspace_char(codeeditor, qtbot):
    """Regression test for issue griffin-ide/griffin#12663."""
    editor = codeeditor
    text = "0123456789\nabcdefghij\n9876543210\njihgfedcba\n"
    editor.set_text(text)
    expected_column = 7
...
def test_qtbug35861(qtbot):
    ...
    If that happens, then the workarounds for griffin-ide/griffin#12663
