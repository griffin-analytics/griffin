...
# Local imports
from griffin.config.base import get_conf_path, running_in_ci
...
def visible_editor_bot(editor_bot, mocker):
    ...
    # message box.
    mocker.patch(
        'griffin.plugins.editor.widgets.editorstack.editorstack.osp.isfile',
        returned_value=True
    )
...
def test_tab_keypress_properly_caught_find_replace(editor_find_replace_bot,
    ...
    Regression test for griffin-ide/griffin#3674.
...
def test_tab_copies_find_to_replace(editor_find_replace_bot, qtbot):
    ...
    keypress. Regression test griffin-ide/griffin#4482."""
    finder = editor_find_replace_bot.find_replace
...
def test_opening_sets_file_hash(base_editor_bot, mocker):
    ...
    editor_stack = base_editor_bot
    mocker.patch('griffin.plugins.editor.widgets.editorstack.editorstack.encoding.read',
...
def test_maybe_autosave_does_not_save_after_open(base_editor_bot, mocker,
    ...
    both Python and text files. The latter covers griffin-ide/griffin#8654.
    """
    editor_stack = base_editor_bot
    mocker.patch('griffin.plugins.editor.widgets.editorstack.editorstack.encoding.read',
                 return_value=('spam\n', 42))
...
def test_maybe_autosave_does_not_save_after_reload(base_editor_bot, mocker):
    ...
    Griffin reloads the file if it has changed on disk. In that case, there is
    no need to autosave because the contents in Griffin are identical to the
    contents on disk.
    """
    editor_stack = base_editor_bot
    txt = 'spam\n'
...
def test_maybe_autosave_handles_error(editor_bot, mocker):
    ...
    mock_dialog = mocker.patch(
        'griffin.plugins.editor.utils.autosave.AutosaveErrorDialog')
    try:
