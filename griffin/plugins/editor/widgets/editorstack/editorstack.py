...
from griffin.config.utils import (
    get_edit_filetypes, get_edit_filters, get_filter, is_kde_desktop
)
from griffin.plugins.editor.api.panel import Panel
from griffin.plugins.editor.utils.autosave import AutosaveForStack
from griffin.plugins.editor.utils.editor import get_file_language
from griffin.plugins.editor.widgets import codeeditor
from griffin.plugins.editor.widgets.editorstack.helpers import (
    ThreadManager, FileInfo, StackHistory)
from griffin.plugins.editor.widgets.tabswitcher import TabSwitcherWidget
from griffin.plugins.explorer.widgets.explorer import (
    show_in_external_file_explorer)
from griffin.plugins.explorer.widgets.utils import fixpath
from griffin.plugins.outlineexplorer.editor import OutlineExplorerProxyEditor
from griffin.plugins.outlineexplorer.api import cell_name
from griffin.plugins.switcher.api import SwitcherActions
from griffin.py3compat import to_text_string
from griffin.utils import encoding, sourcecode, syntaxhighlighters
from griffin.utils.misc import getcwd_or_home
from griffin.utils.palette import GriffinPalette
from griffin.utils.qthelpers import mimedata2url, create_waitspinner
from griffin.utils.stylesheet import PANES_TABBAR_STYLESHEET
from griffin.widgets.tabs import BaseTabs
...
class EditorStack(QWidget, GriffinWidgetMixin):
    ...
    Parameters
    ----------
    codeeditor: griffin.plugins.editor.widgets.codeeditor.CodeEditor
        The codeeditor.
    """
    ...
    See Also
    --------
    :py:meth:griffin.plugins.editor.widgets.editorstack.EditorStack.send_to_help
    """
    ...
    def __init__(self, parent, actions, use_switcher=True):
        if PYQT5 or PYQT6:
            ...
            GriffinWidgetMixin.__init__(self, class_parent=parent)
        ...
        color_scheme = 'griffin/dark' if is_dark_interface() else 'griffin'
    ...
    def show_in_external_file_explorer(self, fnames=None):
        ...
        try:
            ...
            if "xdg-open" in file:
                msg_title = _("Warning")
                msg = _("Griffin can't show this file in the external file "
    ...
    def create_top_widgets(self):
        ...
        css.QToolBar.setValues(
            margin='0px',
            padding='4px',
            borderBottom=f'1px solid {GriffinPalette.COLOR_BACKGROUND_4}'
        )
    ...
    def rename_in_data(self, original_filename, new_filename):
        ...
        if set_new_index:
            ...
            # Fixes griffin-ide/griffin#1287.
    ...
    def set_stack_title(self, index, is_modified):
        ...
        # will happen when changing tabs. See griffin-ide/griffin#1170.
    ...
    def has_filename(self, filename):
        ...
        try:
            ...
            # Fixes griffin-ide/griffin#17685
            try:
    ...
    def set_current_filename(self, filename, focus=True):
        ...
        # Fixes griffin-ide/griffin#15714
        # OSError: This is necessary to catch an error on Windows when Griffin
    ...
    def close_file(self, index=None, force=False):
        ...
        if is_ok:
            ...
            # TODO: Check move of this logic to be part of GriffinMenu itself/be
            ...
            # See griffin-ide/griffin#1094 and griffin-ide/griffin#1098.
            ...
            if editor:
                ...
                # files that were removed outside Griffin one after the other.
                # Fixes griffin-ide/griffin#18838
                QApplication.processEvents()

                # This allows to close files that were removed outside Griffin
    ...
    def save_if_changed(self, cancelable=False, index=None):
        ...
        for index in indexes:
            ...
            # Projects or Files.
            # Fixes griffin-ide/griffin#20998
            try:
                finfo = self.data[index]
            except IndexError:
                return False
    ...
    def _save_file(self, finfo):
        ...
        # have been rearranged. See griffin-ide/griffin#5703.
    ...
    def select_savename(self, original_filename):
        ...
        # slow
        # Fixes griffin-ide/griffin#4156.
    ...
    def save_as(self, index=None):
        ...
        # See griffin-ide/griffin#3678 and griffin-ide/griffin#3026.
    ...
    def current_changed(self, index):
        ...
        # files outside Griffin while in debug mode.
        # See griffin-ide/griffin#8749.
        try:
        ...
        if editor is not None:
            ...
            # that has been renamed. See griffin-ide/griffin#5157.
            try:
    ...
    def _refresh_outlineexplorer(self, index=None, update=True, clear=False):
        ...
        if self.data and len(self.data) > index:
            ...
            if index != self.get_stack_index():
                ...
                # editor stack. See griffin-ide/griffin#8015.
        ...
        if self.data and len(self.data) > index:
            ...
            if not osp.isfile(finfo.filename):
                ...
                try:
                    ...
                    # to write on it
                    # Fixes griffin-ide/griffin#10657
    ...
    def __check_file_status(self, index):
        """
        Check if file has been changed in any way outside Griffin.

        Notes
        -----
        Possible ways are:
        * The file was removed, moved or renamed outside Griffin.
        * The file was modified outside Griffin.
        """
        ...
        if finfo.newly_created:
            ...
            self.msgbox = QMessageBox(
                ...
                _("The file <b>%s</b> is unavailable."
                  "<br><br>"
                  "It may have been removed, moved or renamed outside Griffin."
                  "<br><br>"
            ...
            if str(lastm.toString()) != str(finfo.lastmodified.toString()):
                ...
                # changes in Griffin.
                # Fixes griffin-ide/griffin#21248
                try:
                    ...
                    self.msgbox = QMessageBox(
                        ...
                        self.title,
                        _("The file <b>{}</b> has been modified outside "
                          "Griffin but it was not possible to reload it."
                          "<br><br>"
    ...
    def revert(self):
        ...
        # Fixes griffin-ide/griffin#20284
        try:
            self.reload(index)
    ...
    def load(self, filename, set_current=True, add_where='end',
        ...
        # Fixes griffin-ide/griffin#20670
        try:
            # Read file contents
        ...
        if (
            ...
            self.msgbox = QMessageBox(
                ...
                _("<b>%s</b> contains mixed end-of-line characters.<br>"
                  "Griffin will fix this automatically.") % name,
                QMessageBox.Ok,
                self
            )
            self.msgbox.exec_()
        ...
        # when opening files.
        # Fixes griffin-ide/griffin#20033
        finfo.editor.unhighlight_current_line()
        if self.highlight_current_line_enabled:
            finfo.editor.highlight_current_line()
    ...
    def dragEnterEvent(self, event):
        ...
        # Fixes griffin-ide/griffin#5218.
        ...
        if has_urls and urls and all_urls:
            ...
            # Fixes griffin-ide/griffin#2032.
    ...
    def dropEvent(self, event):
        ...
        # returns None.
        # Fixes griffin-ide/griffin#7742.
