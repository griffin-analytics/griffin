...
from griffin.api.widgets.toolbars import ApplicationToolbar
...
from griffin.plugins.outlineexplorer.main_widget import OutlineExplorerWidget
...
from griffin.utils.qthelpers import create_toolbutton
...
class EditorWidget(QSplitter, GriffinConfigurationObserver):
    ...
    def _generate_splitter_stylesheet(self):
        ...
        css.QSplitter.setValues(
            backgroundColor=GriffinPalette.COLOR_BACKGROUND_1
        )
...
class EditorMainWindow(QMainWindow, GriffinWidgetMixin):
    CONF_SECTION = "editor"
    ...
    def __init__(self, main_widget, menu_actions, outline_plugin, parent=None):
        ...
        # independent. See griffin-ide/griffin#17803
        ...
        self.setWindowTitle("Griffin - %s" % main_widget.windowTitle())
        ...
        for toolbar_id in toolbar_list:
            # This is necessary to run tests for this widget without Griffin's
            # main window
            try:
...
class EditorMainWidgetExample(QSplitter):
    ...
    def register_editorstack(self, editorstack):
        ...
        editorstack.set_default_font(font, color_scheme='Griffin')
...
def test():
    ...
    from griffin.config.base import get_module_path

    griffin_dir = get_module_path('griffin')
    ...
    import time
    t0 = time.time()
    test.load(osp.join(griffin_dir, "widgets", "collectionseditor.py"))
    test.load(osp.join(griffin_dir, "plugins", "editor", "widgets",
                       "window.py"))
    test.load(osp.join(griffin_dir, "plugins", "explorer", "widgets",
                       'explorer.py'))
    test.load(osp.join(griffin_dir, "plugins", "editor", "widgets",
                       "codeeditor", "codeeditor.py"))
    print("Elapsed time: %.3f s" % (time.time()-t0))  # griffin: test-skip
