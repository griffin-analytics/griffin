...
def test_get_files_to_recover_with_empty_autosave_dir(mocker, tmpdir):
    ...
    mocker.patch('griffin.plugins.editor.utils.autosave.get_conf_path',
...
def test_get_files_to_recover_with_one_pid_file(mocker, tmpdir,
    ...
    belongs to a running Griffin instance. If empty is True, then the pid file
    is empty (regression test for griffin-ide/griffin#11375)."""
    mocker.patch('griffin.plugins.editor.utils.autosave.get_conf_path',
                 return_value=str(tmpdir))
    mock_is_griffin_process = mocker.patch(
            'griffin.plugins.editor.utils.autosave.is_griffin_process',
    ...
    mock_is_griffin_process.assert_called_with(42)
...
def test_try_recover(mocker, tmpdir, error_on_remove):
    ...
    mock_RecoveryDialog = mocker.patch(
            'griffin.plugins.editor.utils.autosave.RecoveryDialog')
    mocker.patch('griffin.plugins.editor.utils.autosave.get_conf_path',
                 return_value=str(tmpdir))
    pidfile = tmpdir.join('pid42.txt')
    autosavefile = tmpdir.join('foo.py')
