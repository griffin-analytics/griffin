...
from griffin.config.gui import get_font, is_dark_font_color, set_font
from griffin.config.manager import CONF
from griffin.plugins.appearance.widgets import SchemeEditor
from griffin.utils import syntaxhighlighters
...
class AppearanceConfigPage(PluginConfigPage):
    ...
    def setup_page(self):
        ...
        preview_interface_label_css.QLabel.setValues(
            border=f"1px solid {GriffinPalette.COLOR_BACKGROUND_4}",
    ...
    def get_font(self, option):
        """Return global font used in Griffin."""
        return get_font(option=option)

    def set_font(self, font, option):
        """Set global font used in Griffin."""
    ...
    def delete_scheme(self):
        ...
        if answer == QMessageBox.Yes:
            # Put the combobox in Griffin by default, when deleting a scheme
            names = self.get_option('names')
            default_theme = 'griffin'
            if self.is_dark_interface():
                default_theme = 'griffin/dark'
    ...
    def is_dark_interface(self):
        ...
        """
        return dark_color(GriffinPalette.COLOR_BACKGROUND_1)
    ...
    def color_scheme_and_ui_theme_mismatch(self, color_scheme, ui_theme):
        ...
        ----------
        color_scheme: str
            Name of one of Griffin's color schemes. For instance: 'Zenburn' or
            'Monokai'.
        ui_theme: str
            Name of the one of Griffin's interface themes. This can 'automatic',
            'dark' or 'light'.
        """
