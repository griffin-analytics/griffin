...
from griffin_kernels import __version__ as griffin_kernels_version
from griffin_kernels.utils.pythonenv import is_conda_env
import sympy

# Local imports
from griffin.config.base import running_in_ci, running_in_ci_with_conda
from griffin.config.gui import get_color_scheme
from griffin.py3compat import to_text_string
from griffin.plugins.help.tests.test_plugin import check_text
from griffin.plugins.ipythonconsole.tests.conftest import (
    get_conda_test_env, get_console_background_color, get_console_font_color,
    NEW_DIR, SHELL_TIMEOUT, PY312_OR_GREATER)
from griffin.plugins.ipythonconsole.widgets import ShellWidget
...
def test_dbg_input(ipyconsole, qtbot):
    """Test that griffin doesn't send pdb commands to unrelated input calls."""
    shell = ipyconsole.get_current_shellwidget()
...
def test_restart_kernel(ipyconsole, mocker, qtbot):
    ...
    mocker.patch.object(ShellWidget, "send_griffin_kernel_configuration")
    ...
    # regressions such as griffin-ide/griffin#22084.
    ...
    qtbot.waitUntil(
        lambda: ShellWidget.send_griffin_kernel_configuration.call_count == 2)
...
def test_console_complete(ipyconsole, qtbot, tmpdir):
    ...
    # below.
    # This is a regression test for issue griffin-ide/griffin#20393
    with qtbot.waitSignal(shell.executed):
...
@flaky(max_runs=3)
@pytest.mark.parametrize("griffin_pythonpath", [True, False])
def test_wrong_std_module(ipyconsole, qtbot, tmpdir, griffin_pythonpath):
    """
    ...
    # Assert the extra path from griffin_pythonpath was added
    if griffin_pythonpath:
        check_sys_path = (
    ...
    # Restore CONF
    ipyconsole.set_conf('griffin_pythonpath', [], section='pythonpath_manager')
...
def test_kernel_restart_after_manual_restart_and_crash(ipyconsole, qtbot):
    ...
    This is a regresion for griffin-ide/griffin#12972.
    """
    shell = ipyconsole.get_current_shellwidget()
...
@pytest.mark.skipif(
    parse(griffin_kernels_version) < parse("3.0.0.dev0"),
    reason="Not reliable with Griffin-kernels 2")
def test_shutdown_kernel(ipyconsole, qtbot):
    """
    Check that the kernel is shutdown after creating plots with the
    automatic backend.

    This is a regression test for issue griffin-ide/griffin#17011
    """
    shell = ipyconsole.get_current_shellwidget()
    qtbot.wait(1000)
...
def test_restart_interactive_backend(ipyconsole, qtbot):
    ...
    ipyconsole.restart_kernel()
    qtbot.waitUntil(lambda: shell.griffin_kernel_ready, timeout=SHELL_TIMEOUT)
...
def test_cwd_console_options(ipyconsole, qtbot, tmpdir):
    ...
    def get_cwd_of_new_client():
        ...
        shell = ipyconsole.get_current_shellwidget()
        qtbot.waitUntil(
            lambda: shell.griffin_kernel_ready
...
def test_startup_run_lines_project_directory(ipyconsole, qtbot, tmpdir):
    ...
    # Config griffin_pythonpath with the project path
    ipyconsole.set_conf(
        'griffin_pythonpath',
        [project_dir],
        section='pythonpath_manager')
    ...
    # Reset config for the 'griffin_pythonpath' and 'startup/run_lines'
    ipyconsole.set_conf(
        'griffin_pythonpath',
        [],
        section='pythonpath_manager')
...
def test_old_kernel_version(ipyconsole, qtbot):
    """
    Check that an error is shown when an version of griffin-kernels is used.
    """
    # Set a false _griffin_kernels_version in the cached kernel
    w = ipyconsole.get_widget()

    kernel_handler = w._cached_kernel_properties[-1]
    kernel_handler.kernel_client.sig_griffin_kernel_info.disconnect()
    ...
    # Set wrong version
    kernel_handler.check_griffin_kernel_info(('1.0.0', ''))
    ...
    qtbot.waitUntil(
        lambda: check_text(info_page, "pip install griffin"), timeout=6000
    )
...
def test_run_script(ipyconsole, qtbot, tmp_path):
    ...
    This is a regression test for issue griffin-ide/griffin#15405
    """
...
def test_show_griffin_kernels_error_on_restart(ipyconsole, qtbot):
    """Test that we show Griffin-kernels error message on restarts."""
    # Wait until the window is fully up
    shell = ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
        lambda: shell._prompt_html is not None, timeout=SHELL_TIMEOUT)

    # Point to an interpreter without Griffin-kernels
    ipyconsole.set_conf('default', False, section='main_interpreter')
...
def test_floats_selected_on_double_click(ipyconsole, qtbot):
    ...
    This is a regression test for issue griffin-ide/griffin#22207
    """
    # Wait until the window is fully up
    shell = ipyconsole.get_current_shellwidget()
    qtbot.waitUntil(
...
def test_filter_frames_in_tracebacks(ipyconsole, qtbot, tmp_path):
    ...
    # Check frames related to griffin-kernels are not displayed
    assert "griffin_kernels" not in control.toPlainText()
