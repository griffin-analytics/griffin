...
from griffin.config.base import running_under_pytest
from griffin.plugins.ipythonconsole import (
    GRIFFIN_KERNELS_MIN_VERSION,
    GRIFFIN_KERNELS_MAX_VERSION,
...
from griffin.plugins.ipythonconsole.utils.manager import GriffinKernelManager
from griffin.plugins.ipythonconsole.utils.client import GriffinKernelClient
from griffin.utils.programs import check_version_range
...
ERROR_GRIFFIN_KERNEL_VERSION = _(
    "The Python environment or installation whose interpreter is located at"
    "<pre>"
    "    <tt>{0}</tt>"
    "</pre>"
    "doesn't have the right version of <tt>griffin-kernels</tt> installed ({1} "
    "instead of >= {2} and < {3}). Without this module is not possible for "
    "Griffin to create a console for you.<br><br>"
...
# For Griffin-kernels version < 3.0, where the version and executable cannot be queried
ERROR_GRIFFIN_KERNEL_VERSION_OLD = _(
    "This Python environment doesn't have the right version of "
    "<tt>griffin-kernels</tt> installed (>= {0} and < {1}). Without this "
    "module is not possible for Griffin to create a console for you.<br><br>"
    "You can install it by activating your environment (if necessary) and "
    "then running in a system terminal:"
    "<pre>"
    "    <tt>{2}</tt>"
...
class KernelConnectionState:
    GriffinKernelWaitComm = "griffin_kernel_wait_comm"
    GriffinKernelReady = "griffin_kernel_ready"
...
class KernelHandler(QObject):
    ...
    def __init__(
        ...
        self.known_griffin_kernel = known_griffin_kernel
        ...
        # Start kernel
        self.kernel_client.sig_griffin_kernel_info.connect(
            self.check_griffin_kernel_info
        )
        self.connect_std_pipes()
        self.kernel_client.start_channels()

        # Open comm and wait for comm ready reply.
        # It only works for griffin-kernels, but this is the majority of cases.
    ...
    def connect_(self):
        ...
        if self.connection_state in [
            ...
            # Fixes griffin-ide/griffin#22179
    ...
    def check_griffin_kernel_info(self, griffin_kernel_info):
        """
        Check if the Griffin-kernels version is the right one after receiving it
        from the kernel.

        If the kernel is non-locally managed, check if it is a griffin-kernel.
        """

        if not griffin_kernel_info:
            if self.known_griffin_kernel:
                # griffin-kernels version < 3.0
                self.kernel_error_message = (
                    ERROR_GRIFFIN_KERNEL_VERSION_OLD.format(
                        GRIFFIN_KERNELS_MIN_VERSION,
                        GRIFFIN_KERNELS_MAX_VERSION,
                        GRIFFIN_KERNELS_CONDA,
                        GRIFFIN_KERNELS_PIP,
                ...
                self.known_griffin_kernel = False
                self.sig_kernel_connection_error.emit()
                return
        ...
        version, pyexec = griffin_kernel_info
        if not check_version_range(version, GRIFFIN_KERNELS_VERSION):
            # Development versions are acceptable
            if "dev0" not in version:
                self.kernel_error_message = ERROR_GRIFFIN_KERNEL_VERSION.format(
                    pyexec,
                    version,
                    GRIFFIN_KERNELS_MIN_VERSION,
                    GRIFFIN_KERNELS_MAX_VERSION,
        ...
        self.known_griffin_kernel = True
        self.connection_state = KernelConnectionState.GriffinKernelWaitComm
    ...
    def handle_comm_ready(self):
        ...
        if self.connection_state in [
            KernelConnectionState.GriffinKernelWaitComm,
            ...
            # Fixes griffin-ide/griffin#22179
            self.kernel_client.hb_channel._beating = True
            self.connection_state = KernelConnectionState.GriffinKernelReady
            self.sig_kernel_is_ready.emit()
    ...
    def new_connection_file():
        ...
        """
        # Check if jupyter_runtime_dir exists (Griffin addition)
    ...
    def new_from_spec(cls, kernel_spec):
        ...
        # Kernel manager
        kernel_manager = GriffinKernelManager(
        ...
        try:
            ...
            # writable.
            # Fixes griffin-ide/griffin#23124
            raise GriffinKernelError(
                PERMISSION_ERROR_MSG.format(jupyter_runtime_dir())
            )
    ...
    def init_kernel_client(
        ...
        kernel_client = GriffinKernelClient(connection_file=connection_file)

        # This is needed for issue griffin-ide/griffin#9304.
        try:
    ...
    def copy(self):
        ...
        return self.__class__(
            ...
            known_griffin_kernel=self.known_griffin_kernel,
