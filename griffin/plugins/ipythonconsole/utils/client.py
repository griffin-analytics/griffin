...
class GriffinKernelClient(QtKernelClient):
    # Enable receiving messages on control channel.
    # Useful for pdb completion
    control_channel_class = Type(QtZMQSocketChannel)
    sig_griffin_kernel_info = Signal(object)

    def _handle_kernel_info_reply(self, rep):
        """Check griffin-kernels version."""
        super()._handle_kernel_info_reply(rep)
        griffin_kernels_info = rep["content"].get("griffin_kernels_info", None)
        self.sig_griffin_kernel_info.emit(griffin_kernels_info)
