...
# Local imports
from griffin.config.base import running_in_binder


class GriffinKernelManager(QtKernelManager):
    """
    Griffin kernels that live in a conda environment are now properly activated
    ...
    client_class = DottedObjectName(
        'griffin.plugins.ipythonconsole.utils.client.GriffinKernelClient')
    ...
    async def kill_proc_tree(pid, sig=signal.SIGTERM, include_parent=True,
        ...
        # Fixes griffin-ide/griffin#11872
        try:
        ...
        for child_process in children:
            ...
            # Fixes griffin-ide/griffin#21012
            # Fixes griffin-ide/griffin#13999
            try:
    ...
    async def _async_kill_kernel(self, restart: bool = False) -> None:
        ...
        if self.has_kernel:
            ...
            # kernel started by Griffin.
            # Note: We can't do this in Binder because it freezes Griffin.
            #       Fixes griffin-ide/griffin#22124
