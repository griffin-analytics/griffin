...
from griffin_kernels.utils.pythonenv import PythonEnvInfo, PythonEnvType

# Local imports
from griffin.api.shellconnect.status import ShellConnectStatusBarWidget
...
from griffin.utils.stylesheet import MAC, WIN
...
class MatplotlibStatus(ShellConnectStatusBarWidget):
    ...
    def toggle_matplotlib(self):
        ...
        is_griffin_kernel = sw.is_griffin_kernel

        if not is_griffin_kernel:
    ...
    def config_griffin_kernel(self, shellwidget):
    ...
    def on_kernel_start(self, shellwidget):
        ...
        # `griffin_kernels.comms.commbase.CommError: The comm is not connected.`
        if running_in_ci() and not sys.platform.startswith("linux"):
            mpl_backend = "inline"
        else:
            # Needed when the comm is not connected.
            # Fixes griffin-ide/griffin#22194
            try:
...
class PythonEnvironmentStatus(ShellConnectStatusBarWidget):
    ...
    def show_menu(self):
        ...
        x_offset = (
            ...
            2 * GriffinMenu.HORIZONTAL_MARGIN_FOR_ITEMS
            # Padding of menu items to left and right
            + 2 * GriffinMenu.HORIZONTAL_PADDING_FOR_ITEMS
        )
