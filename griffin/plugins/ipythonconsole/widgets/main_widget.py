...
from griffin.config.base import get_home_dir, running_under_pytest
...
from griffin.plugins.ipythonconsole.widgets.mixins import CachedKernelMixin
from griffin.utils import encoding, sourcecode
from griffin.utils.misc import get_error_match, remove_backslashes
...
from griffin.widgets.printer import GriffinPrinter
...
# ---- Constants
# -----------------------------------------------------------------------------
MAIN_BG_COLOR = GriffinPalette.COLOR_BACKGROUND_1
...
class IPythonConsoleWidget(PluginMainWidget, CachedKernelMixin):
    ...
    Parameters
    ----------
    shellwidget: griffin.plugins.ipyconsole.widgets.shell.ShellWidget
        The shellwigdet.
    """
    ...
    def __init__(self, name=None, plugin=None, parent=None):
        ...
        self.registered_griffin_kernel_handlers = {}
        self.envs = {}
        ...
        if self.enable_infowidget:
            from griffin.widgets.browser import FrameWebView
        ...
        pager_label_css.setValues(**{
            'background-color': f'{GriffinPalette.COLOR_ACCENT_2}',
            'color': f'{GriffinPalette.COLOR_TEXT_1}',
            'margin': '2px 1px 1px 1px',
            'padding': '5px',
            'qproperty-alignment': 'AlignCenter',
            'border-radius': f'{GriffinPalette.SIZE_BORDER_RADIUS}'
        })
        ...
        # Needed to start Griffin in Windows with Python 3.8
        # See griffin-ide/griffin#11880
    ...
    def change_possible_restart_and_mpl_conf(self, option, value):
        ...
        # notification sent when Griffin is starting.
    ...
    def refresh_container(self, give_focus=False):
            ...
            if (client.info_page != client.blank_page and
                ...
                if self.enable_infowidget:
                    try:
                        ...
                        # See griffin-ider/griffin#21509
                        self.enable_infowidget = False
                        self.infowidget = None
        ...
        if client:
            ...
            # directory displayed by other plugins in Griffin (e.g. Files).
    ...
    def update_tabs_text(self):
        ...
        # to an index that doesn't exist. See griffin-ide/griffin#4881.
        try:
    ...
    def config_options(self):
        ...
        # Merge QtConsole and Griffin configs. Griffin prefs will have
    ...
    def create_new_client(self, give_focus=True, filename='', special=None,
        ...
        client = ClientWidget(
            ...
            handlers=self.registered_griffin_kernel_handlers,
    ...
    def close_all_clients(self):
        ...
        # when Griffin is closed.
        # Fixes griffin-ide/griffin#19084
    ...
    def _current_client_print(self):
        ...
        if client:
            ...
            # the app.
            printer = GriffinPrinter(mode=QPrinter.HighResolution)
    ...
    # ---- For kernels
    # -------------------------------------------------------------------------
    def register_griffin_kernel_call_handler(self, handler_id, handler):
        """
        ...
        handler_id : str
            ...
            call the respective handler from the Griffin kernel.
        handler : func
        ...
        None.
        """
        self.registered_griffin_kernel_handlers[handler_id] = handler

    def unregister_griffin_kernel_call_handler(self, handler_id):
        """
        ...
        handler_id : str
            ...
            the Griffin kernel available handlers.

        Returns
        -------
        None.
        """
        self.registered_griffin_kernel_handlers.pop(handler_id, None)
    ...
    def restart_kernel(self, client=None, ask_before_restart=True):
        ...
        if km is None and not client.is_remote():
            client.shellwidget._append_plain_text(
                _('Cannot restart a kernel not started by Griffin\n'),
                before_prompt=True
            )
            return
    ...
    def run_cell(self, code, cell_name, filename, method='runcell'):
        ...
        if client is not None:
            is_griffin_kernel = client.shellwidget.is_griffin_kernel

            if is_griffin_kernel:
                magic_arguments = []
                ...
                # Can not use custom function on non-griffin kernels
                client.shellwidget.append_html_message(
                    _("The console is not running a Griffin-kernel, so it "
                      "can't execute <b>{}</b>.<br><br>"
                      "Please use a Griffin-kernel for this.").format(method),
                    before_prompt=True
                )
                return
    ...
    def run_script(self, filename, wdir, args, post_mortem, current_client,
        ...
        if client is not None:
            if method is None:
                method = "runfile"
            # If griffin-kernels, use runfile
            if client.shellwidget.is_griffin_kernel:
                ...
                # External, non griffin-kernels, use %run
                magic_arguments = []
    ...
    def go_to_error(self, text):
        ...
        if match:
            ...
            # Fixes griffin-ide/griffin#20407
            if '~' in fname:
                fname = osp.expanduser(fname)

            # This is needed to fix issue griffin-ide/griffin#9217.
            try:
