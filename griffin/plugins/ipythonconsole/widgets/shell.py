...
from griffin_kernels.comms.frontendcomm import CommError
...
from griffin.config.base import _, is_conda_based_app, running_under_pytest
from griffin.config.gui import get_color_scheme, is_dark_interface
...
from griffin.plugins.ipythonconsole.utils.style import create_qss_style
from griffin.plugins.ipythonconsole.utils.kernel_handler import (
    KernelConnectionState)
from griffin.plugins.ipythonconsole.widgets import (
...
MODULES_FAQ_URL = (
    "https://docs.griffin-ide.org/5/faq.html#using-packages-installer"
)


class ShellWidget(NamepaceBrowserWidget, HelpWidget, DebuggingWidget,
                  FigureBrowserWidget, GriffinWidgetMixin):
    """
    Shell widget for the IPython Console
    ...
    # Request plugins to send additional configuration to the Griffin kernel
    sig_config_griffin_kernel = Signal()
    ...
    state: dict
        ...
        `GriffinKernel.get_state` method of Griffin-kernels.
    """
    ...
    def __init__(
        ...
        **kw,
        ...
        GriffinWidgetMixin.__init__(self)
        ...
        # set in the qtconsole constructor. See griffin-ide/griffin#4806.
        ...
        # The Qtconsole shortcuts for the actions below don't work in Griffin,
        # so we disable them.
    ...
    # ---- Public API
    @property
    def is_griffin_kernel(self):
        if self.kernel_handler is None:
            return False
        return self.kernel_handler.known_griffin_kernel

    @property
    def griffin_kernel_ready(self):
        """
        Check if Griffin kernel is ready.
        ...
        return (
            ...
            KernelConnectionState.GriffinKernelReady)
    ...
    def call_kernel(self, interrupt=False, blocking=False, callback=None,
        ...
        """
        Send message to Griffin kernel connected to this console.
        ...
        timeout: int or None
            ...
            (defined in commbase.py, present in griffin-kernels) is
            used.
        display_error: bool
    ...
    def setup_griffin_kernel(self):
        """Setup griffin kernel"""
        ...
        self.send_griffin_kernel_configuration()
    ...
    def send_griffin_kernel_configuration(self):
        """Send kernel configuration to griffin kernel."""
        self.is_kernel_configured = False
        ...
        self.sig_config_griffin_kernel.emit()

        if self.is_external_kernel:
            # Enable wurlitzer
            # Not necessary if started by griffin
    ...
    def interrupt_kernel(self):
        ...
        # Fixes griffin-ide/griffin#7293.
        self._execute_queue = []

        if self.griffin_kernel_ready:
            self._reading = False

            # Check if there is a kernel that can be interrupted before trying
            # to do it.
            # Fixes griffin-ide/griffin#20212
            ...
            self._append_html(
                _("<br><br>It is not possible to interrupt a non-Griffin "
    ...
    def set_cwd(self, dirname=None, emit_cwd_change=False):
        ...
        emit_cwd_change: bool
            Whether to emit a Qt signal that informs other panes in Griffin that
    ...
    def send_mpl_backend(self, option=None):
        ...
        """
        if not self.griffin_kernel_ready:
            # will be sent later
            return

        # Set Matplotlib backend with Griffin options
        pylab_n = 'pylab'
        ...
        if pylab_o and backend_o is not None:
            ...
            # Fixes issue griffin-ide/griffin-kernels#108
    ...
    def update_syspath(self, new_paths, prioritize):
        ...
        # Fixes griffin-ide/griffin#21563
        if self.kernel_handler is not None:
...
"""
    ...
    def change_mpl_backend(self, command):
        ...
        Fixes griffin-ide/griffin#4002.
        """
    ...
    def append_html_message(self, html, before_prompt=False,
        ...
        if msg_type == 'error':
            header = _("Error")
            bgcolor = GriffinPalette.COLOR_ERROR_2
        else:
            header = _("Important")
            bgcolor = GriffinPalette.COLOR_WARN_1

        # This makes the header text have good contrast against its background
        # for the light theme.
        if is_dark_interface():
            font_color = GriffinPalette.COLOR_TEXT_1
        else:
            font_color = 'white'
    ...
    def _show_banner(self):
        ...
        # Fixes griffin-ide/griffin#22593
        self.banner = banner
    ...
    def _handle_execute_reply(self, msg):
        """
        Reimplemented to handle communications between Griffin
        and the kernel
        """
    ...
    def _banner_default(self):
        """Override banner creation to handle it in Griffin."""
        return ""
