...
from griffin.utils.installers import InstallerIPythonKernelError
from griffin.utils.environ import RemoteEnvDialog
from griffin.utils.palette import GriffinPalette
from griffin.utils.qthelpers import DialogManager
from griffin.plugins.ipythonconsole import GriffinKernelError
...
from griffin.widgets.mixins import SaveHistoryMixin
...
# Client widget
# ----------------------------------------------------------------------------
class ClientWidget(QWidget, SaveHistoryMixin, GriffinWidgetMixin):
    """
    Client widget for the IPython Console
    ...
    INITHISTORY = ['# -*- coding: utf-8 -*-',
                   '# *** Griffin Python Console History Log ***', ]
    ...
    def _show_special_console_error(self, missing_dependency):
        if missing_dependency is not None:
            error_message = _(
                ...
                "possible for Griffin to create this special console for "
                "you."
    ...
    def show_kernel_error(self, error):
        ...
        if isinstance(error, Exception):
            if isinstance(error, GriffinKernelError):
                error = error.args[0]
            else:
    ...
    def is_benign_error(self, error):
        ...
        benign_errors = [
            ...
            # Old error
            "No such comm",
            # PYDEVD debug warning message. See griffin-ide/griffin#18908
            "Note: Debugging will proceed. "
            "Set PYDEVD_DISABLE_FILE_VALIDATION=1 to disable this validation.",
            # Argument not expected error. See griffin-ide/griffin#19298
    ...
    def close_client(self, is_last_client, close_console=False):
        ...
        # Needed to handle a RuntimeError. See griffin-ide/griffin#5568.
        try:
            # This is required after griffin-ide/griffin#21788 to prevent freezes
            # when closing Griffin. That happens not only when a console is in
    ...
    def finish_close(self, is_last_client, close_console, debugging):
        ...
        # This is a hack to prevent segfaults when closing Griffin and the
        # client was debugging before doing it.
        # It's a side effect of griffin-ide/griffin#21788
    ...
    def interrupt_kernel(self):
        """Interrupt the associanted Griffin kernel if it's running"""
        # Needed to prevent a crash when a kernel is not running.
        # See griffin-ide/griffin#6299.
        try:
    ...
    def show_time(self, end=False):
        ...
        if end:
            ...
            color = GriffinPalette.COLOR_ACCENT_4
