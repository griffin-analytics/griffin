# -*- coding: utf-8 -*-
# ----------------------------------------------------------------------------
# Copyright © 2020- Griffin Project Contributors
#
...
# Pylint plugin below.
# Fixes griffin-ide/griffin#17071
...
from griffin.plugins.pylint.plugin import Pylint
...
# In Python 3 and Griffin 5, replace with enum
NO_DIR = "e"
...
def test_pylint_widget_pylintrc(
    ...
    mocker.patch("os.path.expanduser",
                 return_value=search_paths[HOME_DIR])
    mocker.patch("griffin.plugins.pylint.main_widget.getcwd_or_home",
                 return_value=search_paths[WORKING_DIR])
    mocker.patch("griffin.plugins.pylint.main_widget.osp.expanduser",
                 return_value=search_paths[HOME_DIR])
...
def test_pylint_max_history_conf(pylint_plugin, pylint_test_scripts):
    ...
    For further information see griffin-ide/griffin#12884
    """
    pylint_widget = pylint_plugin.get_widget()
...
def test_get_environment(mocker):
    ...
    if os.name == 'nt':
        mocker.patch(
            "griffin.plugins.pylint.main_widget.is_conda_env",
            return_value=False
        )
        mocker.patch(
            "griffin.plugins.pylint.main_widget.get_home_dir",
            return_value=''
        )
