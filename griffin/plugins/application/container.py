...
Holds references for base actions in the Application of Griffin.
"""
...
# Local imports
from griffin import __docs_url__, __forum_url__, __trouble_url__
from griffin import dependencies
from griffin.api.translations import _
from griffin.api.widgets.main_container import PluginMainContainer
from griffin.utils.installers import InstallerMissingDependencies
from griffin.config.base import get_conf_path, get_debug_level
from griffin.plugins.application.widgets import AboutDialog, InAppAppealStatus
from griffin.plugins.console.api import ConsoleActions
from griffin.utils.environ import UserEnvDialog
from griffin.utils.qthelpers import start_file, DialogManager
...
class LogsMenuSections:
    GriffinLogSection = "griffin_log_section"
...
class ApplicationActions:
    ...
    # 'griffin documentation' is used instead of something
    # like 'griffin_documentation'
    GriffinDocumentationAction = "griffin documentation"
    GriffinDocumentationVideoAction = "griffin_documentation_video_action"
    GriffinTroubleshootingAction = "griffin_troubleshooting_action"
    GriffinDependenciesAction = "griffin_dependencies_action"
    GriffinSupportAction = "griffin_support_action"
    HelpGriffinAction = "help_griffin_action"
    GriffinAbout = "griffin_about_action"

    # Tools
    GriffinUserEnvVariables = "griffin_user_env_variables_action"
    ...
    GriffinRestart = "Restart"
    GriffinRestartDebug = "Restart in debug mode"
...
class ApplicationContainer(PluginMainContainer):
    ...
    def setup(self):
        ...
        griffin_video_url = ("https://www.youtube.com/playlist"
        ...
        self.video_action = self.create_action(
            ...
            triggered=lambda: start_file(griffin_video_url))
        ...
        self.create_action(
            ApplicationActions.HelpGriffinAction,
            _("Help Griffin..."),
        ...
        self.restart_action = self.create_action(
            ApplicationActions.GriffinRestart,
            _("&Restart"),
    ...
    # -------------------------------------------------------------------------
    def on_close(self):
        """To call from Griffin when the plugin is closed."""
    ...
    def show_dependencies(self):
        """Show Griffin Dependencies dialog."""
        ...
        if missing_deps:
            ...
            message = (
                _("<b>You have missing dependencies!</b>"
                  ...
                  "<br><br>"
                  "<i>Note</i>: Griffin could work without some of these "
                  "dependencies, however to have a smooth experience when "
                  "using Griffin we <i>strongly</i> recommend you to install "
    ...
    def restart_debug(self):
        ...
        box.setText(
            _("Which debug mode do you want Griffin to restart in?")
        )
    ...
    def create_debug_log_actions(self):
        ...
        files = [os.environ['GRIFFIN_DEBUG_FILE']]
        ...
        # Add Griffin log on its own section
        self.add_item_to_menu(
            debug_logs_actions[0],
            self.menu_debug_logs,
            section=LogsMenuSections.GriffinLogSection
        )
    ...
    def show_dpi_change_message(self, dpi):
        """Show message to restart Griffin since the DPI scale changed."""
        ...
        if self.current_dpi != dpi:
            ...
            self.dpi_messagebox.setText(
                ...
                ("A monitor scale change was detected. <br><br>"
                 "We recommend restarting Griffin to ensure that it's properly "
                 ...
                 "Interface</tt>, in case Griffin is not displayed "
                 "correctly.<br><br>"
                 "Do you want to restart Griffin?"))
            ...
            # in suspended state. See griffin-ide/griffin#16390
            ...
            # Fixes griffin-ide/griffin#17677
