...
from griffin.plugins.completion.decorators import (
    class_register, handles, request)
from griffin.plugins.explorer.api import DirViewActions
from griffin.plugins.projects.api import (
    BaseProjectType, EmptyProject, WORKSPACE)
from griffin.plugins.projects.utils.watcher import WorkspaceWatcher
from griffin.plugins.projects.widgets.projectdialog import ProjectDialog
from griffin.plugins.projects.widgets.projectexplorer import (
    ProjectExplorerTreeWidget)
from griffin.plugins.switcher.utils import get_file_icon, shorten_paths
...
from griffin.utils.workers import WorkerManager
from griffin.widgets.helperwidgets import PaneEmptyWidget
...
class ProjectExplorerWidget(PluginMainWidget):
        ...
        if project_type_id in project_types:
            ...
            if not running_under_pytest():
                QMessageBox.critical(
                    self,
                    _('Error'),
                    _("<b>{}</b> is not a registered Griffin project "
            ...
            if not self.is_valid_project(path):
                if path:
                    QMessageBox.critical(
                        self,
                        _('Error'),
                        _("<b>%s</b> is not a Griffin project!") % path,
                    )
                return
        else:
            path = encoding.to_unicode_from_fs(path)
    ...
    def delete_project(self):
        ...
        if self.current_active_project:
            ...
            if answer == QMessageBox.Yes:
                try:
                    self.close_project()
                    shutil.rmtree(osp.join(path, '.griffinproject'))
    ...
    def reopen_last_project(self, working_directory, restart_console):
        """
        Reopen the active project when Griffin was closed last time, if any.
        """
    ...
    def is_valid_project(self, path):
        """Check if a directory is a valid Griffin project"""
        spy_project_dir = osp.join(path, '.griffinproject')
            ...
            if not self.is_valid_project(path):
                if path:
                    QMessageBox.critical(
                        self,
                        _('Error'),
                        _("<b>{}</b> is no longer a valid Griffin project! "
    ...
    def register_project_type(self, parent_plugin, project_type):
        ...
        Parameters
        ----------
        parent_plugin: griffin.plugins.api.plugins.GriffinPluginV2
            The parent plugin instance making the project type registration.
        project_type: griffin.plugins.projects.api.BaseProjectType
            Project type to register.
        """
        if not issubclass(project_type, BaseProjectType):
            raise GriffinAPIError("A project type must subclass "
                                 "BaseProjectType!")

        project_id = project_type.ID
        if project_id in self._project_types:
            raise GriffinAPIError("A project type id '{}' has already been "
    ...
    def _load_project_type_class(self, path):
        ...
        Returns
        -------
        griffin.plugins.projects.api.BaseProjectType
        ...
        if osp.isfile(fpath):
            ...
            # Fixes griffin-ide/griffin#17621
            try:
            ...
            # Griffin 4 or older versions.
            # Fixes griffin-ide/griffin#17097
            try:
                ...
                if self.is_valid_project(project):
                    ...
                    try:
                        ...
                        action = self.create_action(
                            name,
                            text=name,
                            icon=self.create_icon('project_griffin'),
