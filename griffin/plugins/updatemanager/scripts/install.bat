@echo off
rem This script updates or installs a new version of Griffin
...
call :wait_for_griffin_quit

IF exist "%conda%" IF exist "%prefix%" (
    call :update_subroutine
    set /P =Press return to exit and launch Griffin...
    call :launch_griffin
    goto exit
)
...
:wait_for_griffin_quit
    echo Waiting for Griffin to quit...
    :loop
    tasklist /v /fi "ImageName eq pythonw.exe" /fo csv 2>NUL | find "Griffin">NUL
    IF "%ERRORLEVEL%"=="0" (
        timeout /t 1 /nobreak > nul
        goto loop
    )
    echo Griffin is quit.
    goto :EOF
...
:install_subroutine
    echo Installing Griffin from: %install_file%

    rem Uninstall Griffin
    for %%I in ("%prefix%\..\..") do set "conda_root=%%~fI"

    echo Install will proceed after the current Griffin version is uninstalled.
    start %conda_root%\Uninstall-Griffin.exe
