...
from griffin.config.base import _, is_conda_based_app, running_in_ci
from griffin.utils.conda import get_griffin_conda_channel
...
CONNECT_ERROR_MSG = _(
    'Unable to connect to the Griffin update service.'
...
SSL_ERROR_MSG = _(
    'SSL certificate verification failed while checking for Griffin updates.'
    '<br><br>Please contact your network administrator for assistance.'
)

OS_ERROR_MSG = _(
    "An error occurred while checking for Griffin updates, possibly related to "
...
def get_asset_info(release: str | Version) -> AssetInfo:
    ...
    if update_type == UpdateType.Major or not is_conda_based_app():
        ...
        if sys.platform.startswith('linux'):
            plat, ext = 'Linux', 'sh'
        name = f'Griffin-{plat}-{mach}.{ext}'
    else:
        name = 'griffin-conda-lock.zip'

    url = (
        'https://github.com/griffin-ide/griffin/releases/download/'
        f'v{release}/{name}'
    )
...
class WorkerUpdate(BaseWorker):
    ...
    blocking the Griffin user interface, in case of connection
    issues.
    """
...
class WorkerDownloadInstaller(BaseWorker):
    ...
    and Linux without blocking the Griffin user interface.
    """
