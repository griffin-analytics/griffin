...
from griffin.plugins.updatemanager.workers import (
    get_asset_info,
    WorkerUpdate,
    WorkerDownloadInstaller
)
from griffin.utils.conda import find_conda, is_anaconda_pkg
from griffin.utils.programs import get_temp_dir, is_program_installed
...
HEADER = _("<h3>Griffin {} is available!</h3><br>")
URL_I = 'https://docs.griffin-ide.org/current/installation.html'
...
class UpdateManagerWidget(QWidget, GriffinConfigurationAccessor):
    ...
    def __init__(self, parent):
        ...
        # Type of Griffin update. It can be "major", "minor" or "micro"
    ...
    def start_check_update(self, startup=False):
        """
        Check for Griffin updates using a QThread.
        ...
        # while loading.
        # Fixes griffin-ide/griffin#15839
        if self.startup:
    ...
    def _process_check_update(self):
        ...
        if (
            ...
            info_messagebox(self, _("Griffin is up to date."), checkbox=True)
    ...
    def _start_download(self):
        ...
        if not self.installer_path.endswith('zip'):
            self.progress_dialog = ProgressDialog(
                self, _("Downloading Griffin {} ...").format(self.latest_release)
            )
    ...
    def _cancel_download(self):
        ...
        box = confirm_messagebox(
            self, msg, _('Griffin download'), critical=True
        )
        if box.result() == QMessageBox.Yes:
            self.cancelled = True
            self.cleanup_threads()
    ...
    def start_install(self):
        ...
        if os.name == 'nt':
            cmd = ['start', '"Update Griffin"'] + sub_cmd
...
class ProgressDialog(UpdateMessageBox):
    ...
    def __init__(self, parent, text):
        ...
        self.setWindowTitle(_("Griffin update"))
...
def error_messagebox(parent, error_msg, checkbox=False):
    ...
    box.setWindowTitle(_("Griffin update error"))
...
def info_messagebox(parent, message, version=None, checkbox=False):
    ...
    box.setWindowTitle(_("New Griffin version"))
    box.setStandardButtons(QMessageBox.Ok)
    box.setDefaultButton(QMessageBox.Ok)
    box.show()
    return box
...
def manual_update_messagebox(parent, latest_release, channel):
    ...
    if is_conda_env(sys.prefix):
        ...
        if is_pypi:
            ...
            msg += (
                "<code>pip install --upgrade griffin</code>"
                "<br><br><br>"
            )

            msg += _(
                "<b>Important note:</b> You installed Griffin with "
            ...
            msg += (
                ...
                f"griffin={latest_release}"
                f"</code><br><br><br>"
            )

            msg += _(
                "<b>Important note:</b> Since you installed "
                "Griffin with Anaconda, please don't use pip "
                "to update it as that will break your "
                "installation."
            )
    else:
        msg += "<code>pip install --upgrade griffin</code><br>"
