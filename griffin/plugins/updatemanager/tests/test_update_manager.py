...
from griffin.plugins.updatemanager import workers
from griffin.plugins.updatemanager.workers import (
    UpdateType, get_asset_info, WorkerUpdate, HTTP_ERROR_MSG
)
from griffin.plugins.updatemanager.widgets import update
from griffin.plugins.updatemanager.widgets.update import UpdateManagerWidget
...
def capture_logging(caplog):
    caplog.set_level(10, "griffin.plugins.updatemanager")
...
def test_updates(qtbot, mocker, caplog, version, channel):
    """
    Test whether or not we offer updates according to the current Griffin
    ...
    mocker.patch.object(
        workers, "get_griffin_conda_channel", return_value=channel
    )

    with caplog.at_level(logging.DEBUG, logger='griffin.plugins.updatemanager'):
        # Capture >=DEBUG logging messages for griffin.plugins.updatemanager
...
def test_download(qtbot, mocker):
    """
    Test download griffin installer.
