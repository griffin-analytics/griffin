...
from griffin.plugins.findinfiles.widgets.main_widget import FindInFilesWidget
from griffin.plugins.findinfiles.widgets.combobox import (
    SearchInComboBox,
    SearchInComboBoxItems
)
from griffin.plugins.findinfiles.widgets.search_thread import SearchThread
...
def searchin_combobox(qtbot, request):
    ...
    from griffin.plugins.findinfiles.widgets import combobox
...
def test_truncate_result_with_different_input(findinfiles, qtbot, line_input):
    ...
    expected_result = (
        f'<span style="color:{GriffinPalette.COLOR_TEXT_1}">'
        f'{line_input_expected[:slice_start]}'
        f'<span style="background-color:{GriffinPalette.COLOR_OCCURRENCE_4}">'
        f'{line_input_expected[slice_start:slice_end]}</span>'
        f'{line_input_expected[slice_end:]}</span>'
    )

    # when
    thread = SearchThread(None, '', text_color=GriffinPalette.COLOR_TEXT_1)
...
def test_no_empty_file_items(findinfiles, qtbot):
    ...
    This is a regression test for issue griffin-ide/griffin#16256
    """
    max_results = 6
    findinfiles.set_search_text("spam")
    findinfiles.set_directory(osp.join(LOCATION, "data"))
...
def test_add_external_paths(searchin_combobox, mocker):
    ...
    mocker.patch(
        'griffin.plugins.findinfiles.widgets.combobox.getexistingdirectory',
...
def test_set_project_path(findinfiles, qtbot):
    ...
    # test, it doesn't need to be a valid Griffin project path.
...
def test_current_search_path(findinfiles, qtbot):
    ...
    # Set the project, file, and griffin path of the SearchInComboBox.
    # For the purpose of this test, the project path doesn't need to be a
    # valid Griffin project path.
...
def test_find_in_single_file(findinfiles, qtbot):
    ...
    This a regression test for issues griffin-ide/griffin#17443 and
    griffin-ide/griffin#20964.
    """
    findinfiles.set_search_text("spam")
