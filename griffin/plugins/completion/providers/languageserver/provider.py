...
from griffin.utils.installers import InstallerPylspError
from griffin.config.base import _, get_conf_path, running_under_pytest
from griffin.config.lsp import PYTHON_CONFIG
from griffin.utils.misc import check_connection_port
from griffin.plugins.completion.api import (SUPPORTED_LANGUAGES,
                                           GriffinCompletionProvider,
                                           WorkspaceUpdateKind)
from griffin.plugins.completion.providers.languageserver.client import LSPClient
from griffin.plugins.completion.providers.languageserver.conftabs import TABS
from griffin.plugins.completion.providers.languageserver.widgets import (
    ClientStatus, LSPStatusWidget, ServerDisabledMessageBox)
from griffin.utils.introspection.module_completion import PREFERRED_MODULES
...
class LanguageServerProvider(GriffinCompletionProvider):
    ...
    def get_root_path(self, language):
        ...
        if not path:
            ...
            # just like we did for Rope in Griffin 3.
    ...
    def report_lsp_down(self, language):
        ...
        if os.name == 'nt':
            os_message = (
                ...
                "system, or restart Griffin.<br><br>"
            )
        else:
            os_message = (
                "This problem could be fixed by restarting Griffin. "
            )
        ...
        warn_str = (
            ...
            + os_message +
            _("Do you want to restart Griffin now?")
        )
    ...
    def python_path_update(self, new_path, prioritize):
        """
        Update server configuration after a change in Griffin's Python
        path.

        Parameters
        ----------
        new_path: list of str
            New state of the Python path handled by Griffin.
        prioritize: bool
    ...
    def on_pyls_griffin_configuration_change(self, option, value):
        self.update_lsp_configuration()
    ...
    @on_conf_change(
        ...
        option=['griffin_pythonpath', 'prioritize']
    )
    ...
    def generate_python_config(self):
        ...
        jedi = {
            ...
            'extra_paths': self.get_conf('griffin_pythonpath',
                                         section='pythonpath_manager',
                                         default=[]),
        ...
        plugins['pyls_griffin'].update(pyls_griffin_options)
