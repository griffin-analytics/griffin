...
"""
API to create an entry in Griffin Preferences associated to a given plugin.
"""
...
from griffin.widgets.config import GriffinConfigPage, BaseConfigTab


OptionSet = Set[ConfigurationKey]


class GriffinPreferencesTab(BaseConfigTab):
    """
    Widget that represents a tab on a preference page.

    All calls to :class:`GriffinConfigPage` attributes are resolved
    via delegation.
    """
    ...
    def __init__(self, parent: GriffinConfigPage):
...
class PluginConfigPage(GriffinConfigPage):
    """
    Widget to expose the options a plugin offers for configuration as
    an entry in Griffin's Preferences dialog.
    """
    ...
    def __init__(self, plugin, parent):
        ...
        GriffinConfigPage.__init__(self, parent)

    def _wrap_apply_settings(self, func):
        """
        Wrap apply_settings call to ensure that a user-defined custom call
        is called alongside the Griffin Plugin API configuration propagation
        call.
        """
