...
from griffin.config.types import ConfigurationKey
...
class GriffinConfigurationAccessor:
    """
    Mixin used to access options stored in the Griffin configuration system.
    """

    # Name of the configuration section that's going to be
    # used to record the object's permanent data in Griffin
    # config system.
    CONF_SECTION = None
    ...
    def get_conf(
        ...
        """
        Get an option from the Griffin configuration system.
        ...
        if section is None:
            raise AttributeError(
                'A GriffinConfigurationAccessor must define a `CONF_SECTION` '
                'class attribute!'
            )
    ...
    def set_conf(
        ...
        """
        Set an option in the Griffin configuration system.
    ...
    def remove_conf(
        ...
        """
        Remove an option in the Griffin configuration system.
    ...
    def get_conf_default(self,
                         ...
                         section: Optional[str] = None):
        """
        Get an option default value in the Griffin configuration system.
    ...
    def old_conf_version(self):
        """Get old Griffin configuration version."""
        return CONF.old_griffin_version


class GriffinConfigurationObserver(GriffinConfigurationAccessor):
    """
    Concrete implementation of the protocol
    :class:`griffin.config.types.ConfigurationObserver`.

    This mixin enables a class to receive configuration updates seamlessly,
    by registering methods using the
    :function:`griffin.api.config.decorators.on_conf_change` decorator, which
    ...
    def __init__(self):
        ...
        if self.CONF_SECTION is None:
            warnings.warn(
                'A GriffinConfigurationObserver must define a `CONF_SECTION` '
    ...
    def add_configuration_observer(
        ...
        - This is only necessary if you need to add a callable that is not a
          ...
          :function:`griffin.api.config.decorators.on_conf_change`.
        """
        if section is None:
