...
# Copyright (c) 2016- Griffin Project Contributors (see AUTHORS.txt)
#
...
# Local imports
from griffin.api.config.mixins import (
    GriffinConfigurationAccessor,
    GriffinConfigurationObserver
)
...
from griffin.api.widgets.toolbars import GriffinToolbar
...
from griffin.utils.qthelpers import create_action, create_toolbutton
from griffin.utils.registries import (
    ACTION_REGISTRY, MENU_REGISTRY, TOOLBAR_REGISTRY, TOOLBUTTON_REGISTRY)
from griffin.utils.stylesheet import PANES_TOOLBAR_STYLESHEET


class GriffinToolButtonMixin:
    """
...
class GriffinToolbarMixin:
    ...
    def create_toolbar(
        ...
        """
        toolbar = GriffinToolbar(self, name)
...
class GriffinMenuMixin:
    ...
    def add_item_to_menu(self, action_or_menu, menu, section=None,
        ...
        """
        if not isinstance(menu, GriffinMenu):
            raise GriffinAPIError('Menu must be an instance of GriffinMenu!')
    ...
    def _create_menu(
        ...
        MenuClass=GriffinMenu
    ) -> GriffinMenu:
        """
        Create a GriffinMenu or a subclass of it.

        Notes
        -----
        * This method must only be used directly to generate a menu that is a
          subclass of GriffinMenu.
        * Refer to the documentation for `GriffinMenu` to learn about its args.
        """
        if register:
    ...
    def get_menus(self, context: Optional[str] = None,
                  plugin: Optional[str] = None) -> Dict[str, GriffinMenu]:
        """
...
class GriffinActionMixin:
    ...
    def get_actions(self, context: Optional[str] = None,
        ...
        Returns
        -------
        actions: Dict[str, GriffinAction]
        ...
        2. Actions can be created directly on a PluginMainWidget or
           ...
           actions, but they need to subclass GriffinWidgetMixin.
...
class GriffinWidgetMixin(
    GriffinActionMixin,
    GriffinMenuMixin,
    GriffinToolbarMixin,
    GriffinToolButtonMixin,
    GriffinShortcutsMixin,
):
    """
    Basic functionality for all Griffin widgets and Qt items.
    ...
    def create_icon(name):
        """
        Create an icon by name using the griffin Icon manager.
        """
...
class GriffinMainWindowMixin:
    """
    Mixin with additional functionality for the QMainWindow's used in Griffin.
    """
    ...
    def move_to_primary_screen(self: QMainWindow):
        ...
        # With this we want to maximize only the Griffin main window and not the
        # plugin ones, which usually are not maximized.
        if not hasattr(self, 'is_window_widget'):
            self.showMaximized()


class SvgToScaledPixmap(GriffinConfigurationAccessor):
    """
    ...
    def svg_to_scaled_pixmap(self, svg_file, rescale=None, in_package=True):
        ...
        in_package: bool, optional
            Get svg from the `images` folder in the Griffin package.
        """
        if in_package:
