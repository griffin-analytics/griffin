...
"""
Griffin combobox widgets.

Use these widgets for any combobox you want to add to Griffin.
"""
...
from griffin.utils.stylesheet import AppStyle, WIN


class _GriffinComboBoxProxyStyle(QProxyStyle):
    """Style proxy to adjust qdarkstyle issues."""

    def styleHint(self, hint, option=None, widget=None, returnData=None):
        if hint == QStyle.SH_ComboBox_Popup:
            # Disable combobox popup top & bottom areas.
            # See griffin-ide/griffin#9682.
...
class _GriffinComboBoxDelegate(QStyledItemDelegate):
    """
    ...
    def paint(self, painter, option, index):
        ...
        if data and data == "separator":
            painter.setPen(QColor(GriffinPalette.COLOR_BACKGROUND_6))
            painter.drawLine(
...
class _GriffinComboBoxLineEdit(QLineEdit):
    ...
    def paintEvent(self, event):
        if self._elide_mode is not None and not self._focus_in:
            ...
            color = (
                GriffinPalette.COLOR_TEXT_1
                if self.isEnabled()
                else GriffinPalette.COLOR_DISABLED
            )
...
class _GriffinComboBoxView(QListView):
...
class _GriffinComboBoxMixin:
    ...
    def _generate_stylesheet(self):
        """Base stylesheet for Griffin comboboxes."""
        css = qstylizer.style.StyleSheet()
        ...
        css["QComboBox QAbstractItemView::item:selected:active"].setValues(
            backgroundColor=GriffinPalette.COLOR_BACKGROUND_3,
        )

        return css


class GriffinComboBox(QComboBox, _GriffinComboBoxMixin):
    """Default combobox widget for Griffin."""
    ...
    def __init__(self, parent=None, items_elide_mode=None):
        ...
        if PYQT5 or PYQT6:
            ...
            _GriffinComboBoxMixin.__init__(self)
        ...
        self.setItemDelegate(
            _GriffinComboBoxDelegate(self, elide_mode=items_elide_mode)
        )
    ...
    def showEvent(self, event):
        ...
        if not self._is_shown:
            if not self.isEditable():
                ...
                self.setLineEdit(_GriffinComboBoxLineEdit(self, editable=False))
...
class GriffinComboBoxWithIcons(GriffinComboBox):
    """"Combobox widget for Griffin when its items have icons."""
...
class GriffinFontComboBox(QFontComboBox, _GriffinComboBoxMixin):
    ...
    def __init__(self, parent=None):
        ...
        # Fixes griffin-ide/griffin#22683
        self.setItemDelegate(
            _GriffinComboBoxDelegate(self, elide_mode=Qt.ElideNone)
        )

        # Elide selected font name in case it's too long
        self.setLineEdit(
            _GriffinComboBoxLineEdit(
