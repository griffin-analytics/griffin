...
from griffin.utils.qthelpers import add_actions, set_menu_icons, GriffinAction
from griffin.utils.palette import GriffinPalette
from griffin.utils.stylesheet import AppStyle, MAC, WIN
...
# ---- Style
# -----------------------------------------------------------------------------
class GriffinMenuProxyStyle(QProxyStyle):
...
# ---- Widgets
# -----------------------------------------------------------------------------
class GriffinMenu(QMenu, GriffinFontsMixin):
    """
    A QMenu subclass to implement additional functionality for Griffin.
    """
    MENUS = []
    APP_MENU = False
    ...
    def __init__(
        ...
        style = GriffinMenuProxyStyle(None)
    ...
    def add_action(self: T,
        ...
        """
        item_id = None
        if isinstance(action, GriffinAction) or hasattr(action, 'action_id'):
            item_id = action.action_id

            # This is necessary when we set a menu for `action`, e.g. for
            # todo_list_action in EditorMainWidget.
            if action.menu() and isinstance(action.menu(), GriffinMenu):
                action.menu()._is_submenu = True
        elif isinstance(action, GriffinMenu) or hasattr(action, 'menu_id'):
    ...
    def _generate_stylesheet(cls):
        ...
        font = cls.get_font(GriffinFontType.Interface)
        ...
        css.QMenu.setValues(
            ...
            border=f"1px solid {GriffinPalette.COLOR_BACKGROUND_6}"
        )
        ...
        for state in ['selected', 'pressed']:
            if state == 'selected':
                bg_color = GriffinPalette.COLOR_BACKGROUND_4
            else:
        ...
        for state in ['disabled', 'selected:disabled']:
            css[f"QMenu::item:{state}"].setValues(
                color=GriffinPalette.COLOR_DISABLED,
                backgroundColor="transparent"
            )
    ...
    def __str__(self):
        return f"GriffinMenu('{self.menu_id}')"
    ...
    def showEvent(self, event):
        ...
        # (as in griffin-ide/griffin#22266), we use a timer to queue the move
...
class PluginMainWidgetOptionsMenu(GriffinMenu):
    """
