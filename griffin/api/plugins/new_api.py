...
All plugins in Griffin 5+ must inherit from the classes present in this file.
"""
...
# Local imports
from griffin.api.config.mixins import GriffinConfigurationObserver
from griffin.api.exceptions import GriffinAPIError
from griffin.api.fonts import GriffinFontType
from griffin.api.plugin_registration.mixins import GriffinPluginObserver
from griffin.api.widgets.main_widget import PluginMainWidget
from griffin.api.widgets.mixins import GriffinActionMixin
from griffin.api.widgets.mixins import GriffinWidgetMixin
from griffin.app.cli_options import get_options
from griffin.config.gui import get_color_scheme, get_font
from griffin.config.user import NoDefault
...
from ._old_api import GriffinPluginWidget
...
class GriffinPluginV2(QObject, GriffinActionMixin, GriffinConfigurationObserver,
                     GriffinPluginObserver):
    """
    A Griffin plugin to extend functionality without a dockable widget.

    If you want to create a plugin that adds a new pane, please use
    GriffinDockablePlugin.
    """
    ...
    # used to record the plugin's permanent data in Griffin
    # config system (i.e. in griffin.ini)
    CONF_SECTION = None
    ...
    # Widget to be used as entry in Griffin Preferences dialog.
    ...
    # inherit from `GriffinPreferencesTab`.
    # Example:
    # ADDITIONAL_CONF_TABS = {'plugin_name': [<GriffinPreferencesTab classes>]}
    ...
    #       `griffin.pluginsl.layout.api::BaseGridLayoutType`
    CUSTOM_LAYOUTS = []

    # Path for images relative to the plugin path
    # A Python package can include one or several Griffin plugins. In this case
    # the package may be using images from a global folder outside the plugin
    # folder
    IMG_PATH = None

    # Control the font size relative to the global fonts defined in Griffin
    ...
    # web widgets to enhance the distribution of Griffin to users
    # https://github.com/griffin-ide/griffin/pull/22196#issuecomment-2189377043
    REQUIRE_WEB_WIDGETS = False

    # --- API: Signals -------------------------------------------------------
    # ------------------------------------------------------------------------
    # Signals here are automatically connected by the Griffin main window and
    ...
    Parameters
    ----------
    plugin_instance: GriffinDockablePlugin
    ...
    # among several files. See griffin/config/main.py
    _CONF_NAME_MAP = None
    ...
    def __init__(self, parent, configuration=None):
        ...
        # GriffinPluginObserver and GriffinConfigurationObserver when using
        # super(), see https://fuhm.net/super-harmful/
        GriffinPluginObserver.__init__(self)
        GriffinConfigurationObserver.__init__(self)
        ...
        if self.CONTAINER_CLASS is not None:
            ...
            if isinstance(container, GriffinWidgetMixin):
                container.setup()
    ...
    def _register(self, omit_conf=False):
        """
        Setup and register plugin in Griffin's main window and connect it to
        other plugins.
        """
        # Checks
        # --------------------------------------------------------------------
        if self.NAME is None:
            raise GriffinAPIError('A Griffin Plugin must define a `NAME`!')
    ...
    def get_configuration(self):
        """
        Return the Griffin configuration object.
        """
        return self._conf

    def get_main(self):
        """
        Return the Griffin main window..
        """
        return self._main

    def get_plugin(self, plugin_name, error=True) -> GriffinPluginV2:
        """
    ...
    def get_dockable_plugins(self):
        ...
        Only required plugins that extend GriffinDockablePlugin are returned.
        """
        requires = set(self.REQUIRES or [])
        ...
        for name, plugin_instance in self._main.get_dockable_plugins():
            if (name in requires or Plugins.All in requires) and isinstance(
                    plugin_instance,
                    (GriffinDockablePlugin, GriffinPluginWidget)):
    ...
    def get_conf(self, option, default=NoDefault, section=None, secure=False):
        """
        Get an option from Griffin configuration system.
        ...
        if self._conf is not None:
            ...
            if section is None:
                raise GriffinAPIError(
                    'A griffin plugin must define a `CONF_SECTION` class '
                    'attribute!'
                )
    ...
    def set_conf(
        ...
        """
        Set an option in Griffin configuration system.
    ...
    def remove_conf(self, option, section=None, secure=False):
        """
        Delete an option in the Griffin configuration system.
    ...
    def disable_conf(self, option, section=None):
        """
        Disable notifications for an option in the Griffin configuration system.
    ...
    def restore_conf(self, option, section=None):
        """
        Restore notifications for an option in the Griffin configuration system.
    ...
    def get_font(cls, font_type):
        """
        Return one of font types used in Griffin.

        Parameters
        ----------
        font_type: str
            There are three types of font types in Griffin:
            GriffinFontType.Monospace, used in the Editor, IPython console,
            and History; GriffinFontType.Interface, used by the entire Griffin
            app; and GriffinFontType.MonospaceInterface, used by the Variable
        ...
        Notes
        -----
        All plugins in Griffin use the same, global fonts. In case some a plugin
        ...
        constants.
        """
        if font_type == GriffinFontType.Monospace:
            font_size_delta = cls.MONOSPACE_FONT_SIZE_DELTA
        elif font_type in [GriffinFontType.Interface,
                           GriffinFontType.MonospaceInterface]:
            font_size_delta = cls.INTERFACE_FONT_SIZE_DELTA
        else:
            raise GriffinAPIError("Unrecognized font type")
    ...
    def on_initialize(self):
        ...
        if hasattr(self, 'register'):
            raise GriffinAPIError(
                ...
                'Griffin 5.1.0 migration guide to get more information')
    ...
    def check_compatibility():
        ...
        -------
        (bool, str)
            The first value tells Griffin if the plugin has passed the
    ...
    def on_close(self, cancelable=False):
        ...
        if hasattr(self, 'unregister'):
            warnings.warn('The unregister method was deprecated and it '
                          ...
                          'Griffin 5.2.0 migration guide to get more '
                          'information.')
    ...
    def update_font(self):
        ...
        The following plugins illustrate the usage of this method:
          * griffin/plugins/help/plugin.py
          * griffin/plugins/onlinehelp/plugin.py
        """
        pass
...
class GriffinDockablePlugin(GriffinPluginV2):
    """
    A Griffin plugin to enhance functionality with a dockable widget.
    """
    # ---- API: Mandatory attributes
    # -------------------------------------------------------------------------
    ...
    # Disable actions in Griffin main menus when the plugin is not visible
    ...
    def __init__(self, parent, configuration):
        if not issubclass(self.WIDGET_CLASS, PluginMainWidget):
            raise GriffinAPIError(
                'A GriffinDockablePlugin must define a valid WIDGET_CLASS '
                'attribute!')
    ...
    def get_widget(self) -> PluginMainWidget:
        ...
        if self._widget is None:
            raise GriffinAPIError('Dockable Plugin must have a WIDGET_CLASS!')
