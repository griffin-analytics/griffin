...
These plugins were supported until Griffin 5, but that's no longer the case.
...
from griffin.plugins.base import BasePluginMixin, BasePluginWidgetMixin
...
class BasePlugin(BasePluginMixin):
    """
    Basic functionality for Griffin plugins.
    ...
    def set_option(self, option, value, section=None,
        ...
        """
        Set an option in Griffin configuration file.
    ...
    def get_option(self, option, default=NoDefault, section=None):
        """
        Get an option from Griffin configuration file.
    ...
    def remove_option(self, option, section=None):
        """
        Remove an option from the Griffin configuration file.
...
class GriffinPlugin(BasePlugin):
    """
    Griffin plugin class.
...
class BasePluginWidget(QWidget, BasePluginWidgetMixin):
    """
    Basic functionality for Griffin plugin widgets.
    ...
    def __init__(self, main=None):
        ...
        # displayed in Griffin for this plugin.
    ...
    def tabify(self, core_plugin):
        ...
        Parameters
        ----------
        core_plugin: GriffinPluginWidget
            Core Griffin plugin this one will be tabified next to.
    ...
    def get_font(self, rich_text=False):
        ...
        Notes
        -----
        All plugins in Griffin use the same, global font. This is a
    ...
    def register_widget_shortcuts(self, widget):
        ...
        `griffin/widgets/findreplace.py` for an example.
        """
...
class GriffinPluginWidget(GriffinPlugin, BasePluginWidget):
    """
    Griffin plugin widget class.
    ...
    # relative to the fonts defined in Griffin
    # Status: Optional
    FONT_SIZE_DELTA = 0
    RICH_FONT_SIZE_DELTA = 0

    # Disable actions in Griffin main menus when the plugin
    # is not visible
    # Status: Optional
    DISABLE_ACTIONS_WHEN_HIDDEN = True

    # Shortcut to give focus to the plugin. In Griffin we try
    ...
    def register_plugin(self):
        """
        Register plugin in Griffin's main window and connect it to other
        plugins.
    ...
    def toggle_view(self, checked):
        ...
        `griffin/plugins/ipythonconsole/plugin.py`
        """
