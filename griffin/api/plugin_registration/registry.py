...
# TODO: Remove GriffinPlugin and GriffinPluginWidget now that the migration
# is complete
Griffin5PluginClass = Union[GriffinPluginV2, GriffinDockablePlugin]
Griffin4PluginClass = Union[GriffinPlugin, GriffinPluginWidget]
GriffinPluginClass = Union[Griffin4PluginClass, Griffin5PluginClass]
...
class PreferencesAdapter(GriffinConfigurationAccessor):
...
class GriffinPluginRegistry(QObject, PreferencesAdapter):
    """
    Global plugin registry.
    ...
    def __init__(self):
        ...
        self.plugin_registry = {}  # type: Dict[str, GriffinPluginClass]

        # Dictionary that maps a plugin name to its availability.
        self.plugin_availability = {}  # type: Dict[str, bool]

        # Set that stores the plugin names of all Griffin 4 plugins.
        ...
        self.all_internal_plugins = {}  # type: Dict[str, Tuple[str, Type[GriffinPluginClass]]]

        # Dictionary that contains all the external plugins (enabled or not)
        self.all_external_plugins = {}  # type: Dict[str, Tuple[str, Type[GriffinPluginClass]]]
    ...
    def _instantiate_griffin5_plugin(
            self, main_window: Any,
            PluginClass: Type[Griffin5PluginClass],
            external: bool) -> Griffin5PluginClass:
        """Instantiate and register a Griffin 5+ plugin."""
        ...
        if external:
            # These attributes come from griffin.app.find_plugins
            module = PluginClass._griffin_module_name
            package_name = PluginClass._griffin_package_name
            version = PluginClass._griffin_version
    ...
    def _instantiate_griffin4_plugin(
            self, main_window: Any,
            PluginClass: Type[Griffin4PluginClass],
            external: bool,
            args: tuple, kwargs: dict) -> Griffin4PluginClass:
        """Instantiate and register a Griffin 4 plugin."""
        plugin_name = PluginClass.NAME
        ...
        # Since Griffin 5+ plugins are loaded before old ones, preferences
    ...
    def register_plugin(
            self, main_window: Any,
            PluginClass: Type[GriffinPluginClass],
            *args: tuple, external: bool = False,
            **kwargs: dict) -> GriffinPluginClass:
        """
        Register a plugin into the Griffin registry.

        Parameters
        ----------
        main_window: griffin.app.mainwindow.MainWindow
            Reference to Griffin's main window.
        PluginClass: type[GriffinPluginClass]
            The plugin class to register and create. It must be one of
            `griffin.app.registry.GriffinPluginClass`.
        *args: tuple
        ...
        **kwargs: dict
        ...
        """
        if not issubclass(PluginClass, (GriffinPluginV2, GriffinPlugin)):
            raise TypeError(f'{PluginClass} does not inherit from '
                            f'{GriffinPluginV2} nor {GriffinPlugin}')

        instance = None
        if issubclass(PluginClass, GriffinPluginV2):
            # Register a Griffin 5+ plugin
            instance = self._instantiate_griffin5_plugin(
                main_window, PluginClass, external)
        elif issubclass(PluginClass, GriffinPlugin):
            # Register a Griffin 4 plugin
            instance = self._instantiate_griffin4_plugin(
    ...
    def notify_plugin_availability(self, plugin_name: str,
        ...
        omit_conf: bool
            ...
            plugin configuration into the Griffin configuration file.
        """
    ...
    def dock_undocked_plugin(
        ...
        elif isinstance(plugin_instance, GriffinPluginWidget):
    ...
    def delete_plugin(self, plugin_name: str, teardown: bool = True,
        ...
        if isinstance(plugin_instance, GriffinPluginV2):
            ...
            try:
                ...
                pass
        elif isinstance(plugin_instance, GriffinPlugin):
            try:
                plugin_instance.deleteLater()
            except RuntimeError:
                pass
    ...
    def delete_all_plugins(self, excluding: Optional[Set[str]] = None,
        ...
        internal ones, where the Griffin 4 plugins will be removed first and
        then the Griffin 5 ones.
        ...
        # Delete Griffin 4 external plugins
        for plugin_name in set(self.external_plugins):
            if plugin_name not in excluding:
                plugin_instance = self.plugin_registry[plugin_name]
                if isinstance(plugin_instance, GriffinPlugin):
        ...
        # Delete Griffin 4 internal plugins
        ...
        # Delete Griffin 5+ external plugins
        for plugin_name in set(self.external_plugins):
            if plugin_name not in excluding:
                plugin_instance = self.plugin_registry[plugin_name]
                if isinstance(plugin_instance, GriffinPluginV2):
        ...
        # Delete Griffin 5 internal plugins
        for plugin_name in set(self.internal_plugins):
            if plugin_name not in excluding:
                plugin_instance = self.plugin_registry[plugin_name]
                if isinstance(plugin_instance, GriffinPluginV2):
    ...
    def get_plugin(self, plugin_name: str) -> GriffinPluginClass:
        """
        ...
        if plugin_name in self.plugin_registry:
            ...
            raise GriffinAPIError(f'Plugin {plugin_name} was not found in '
                                 'the registry')
    ...
    def set_all_internal_plugins(
            self, all_plugins: Dict[str, Type[GriffinPluginClass]]):
...
PLUGIN_REGISTRY = GriffinPluginRegistry()
