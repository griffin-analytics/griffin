...
# Local imports
from griffin.dependencies import DESCRIPTIONS, OPTIONAL, PY38
...
def parse_griffin_dependencies():
    """
    Parse Griffin dependencies and return a dict of deps and versions.
    """
    deps = {}
...
def parse_setup_install_requires(fpath):
    """
    Parse Griffin setup.py and return a dict of deps and versions.
    """
    with open(fpath, 'r') as fh:
        data = fh.read()
...
def test_dependencies_for_binder_in_sync():
    ...
    requirements.
    """
    griffin_env = parse_environment_yaml(ENV_FPATH)
    ...
    assert griffin_env == full_reqs


@pytest.mark.skipif(PY38, reason="Fails in Python 3.8")
def test_dependencies_for_griffin_dialog_in_sync():
    """
    Griffin dependencies dialog should share deps with main.yml.
    """
    griffin_deps = parse_griffin_dependencies()
    main_reqs = parse_environment_yaml(REQ_FPATH)
    ...
    assert griffin_deps == full_reqs


def test_dependencies_for_griffin_setup_install_requires_in_sync():
    """
    Griffin setup.py should share deps with main.yml.
    """
    griffin_setup = parse_setup_install_requires(SETUP_FPATH)
    main_reqs = parse_environment_yaml(REQ_FPATH)
    windows_reqs = parse_environment_yaml(REQ_WINDOWS_FPATH)
    mac_reqs = parse_environment_yaml(REQ_MAC_FPATH)
    linux_reqs = parse_environment_yaml(REQ_LINUX_FPATH)
    ...
    griffin_setup.pop('pyqt5-sip')

    assert griffin_setup == full_reqs


def test_dependencies_for_griffin_setup_extras_requires_in_sync():
    """
    Griffin setup.py extra_requires should share deps with tests.yml.
    """
    griffin_extras_setup = parse_setup_extra_requires(SETUP_FPATH)
    griffin_test_reqs = parse_environment_yaml(REQ_TEST_FPATH)

    assert griffin_extras_setup == griffin_test_reqs
