...
from griffin_kernels.utils.misc import fix_reference_name
...
# Local imports
from griffin.api.fonts import GriffinFontsMixin, GriffinFontType
from griffin.api.widgets.mixins import GriffinWidgetMixin
from griffin.config.base import _, running_under_pytest
from griffin.py3compat import (is_binary_string, to_text_string,
                              is_type_text_string)
from griffin.utils.icon_manager import ima
from griffin.utils.misc import getcwd_or_home
from griffin.utils.qthelpers import mimedata2url
from griffin.utils.stringmatching import get_search_scores, get_search_regex
from griffin.plugins.variableexplorer.widgets.collectionsdelegate import (
    CollectionsDelegate,
    SELECT_ROW_BUTTON_SIZE,
)
from griffin.plugins.variableexplorer.widgets.importwizard import ImportWizard
from griffin.widgets.helperwidgets import CustomSortFilterProxy
...
from griffin.utils.stylesheet import AppStyle, MAC
...
class ProxyObject(object):
    ...
    def __getitem__(self, key):
        ...
        # Catch NotImplementedError to fix griffin-ide/griffin#6284 in pandas
        # MultiIndex due to NA checking not being supported on a multiindex.
        # Catch AttributeError to fix griffin-ide/griffin#5642 in certain special
        ...
        # modifying certain pandas objects. Fix griffin-ide/griffin#6727.
        # Catch ValueError to allow viewing and editing of pandas offsets.
        # Fix griffin-ide/griffin#6728-
        try:
...
# ---- Widgets
# =============================================================================
class ReadOnlyCollectionsModel(QAbstractTableModel, GriffinFontsMixin):
    ...
    def set_data(self, data, coll_filter=None):
        ...
        if isinstance(data, tuple):
            ...
            try:
                ...
                # types as keys.
                # Fixes griffin-ide/griffin#13481
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.UserRole:
            ...
            return to_qvariant(
                self.get_font(GriffinFontType.MonospaceInterface)
            )
        return to_qvariant()
...
class CollectionsModel(ReadOnlyCollectionsModel):
    ...
    def type_to_color(self, python_type, numpy_type):
        ...
        color = GriffinPalette.GROUP_12

        if numpy_type != 'Unknown':
            if numpy_type == 'Array':
                color = GriffinPalette.GROUP_9
            elif numpy_type == 'Scalar':
                color = GriffinPalette.GROUP_2
        elif python_type == 'bool':
            color = GriffinPalette.GROUP_1
        elif python_type in ['int', 'float', 'complex']:
            color = GriffinPalette.GROUP_2
        elif python_type in ['str', 'unicode']:
            color = GriffinPalette.GROUP_3
        elif 'datetime' in python_type:
            color = GriffinPalette.GROUP_4
        elif python_type == 'list':
            color = GriffinPalette.GROUP_5
        elif python_type == 'set':
            color = GriffinPalette.GROUP_6
        elif python_type == 'tuple':
            color = GriffinPalette.GROUP_7
        elif python_type == 'dict':
            color = GriffinPalette.GROUP_8
        elif python_type in ['MaskedArray', 'Matrix', 'NDArray']:
            color = GriffinPalette.GROUP_9
        elif (python_type in ['DataFrame', 'Series'] or
                'Index' in python_type):
            color = GriffinPalette.GROUP_10
        elif python_type == 'PIL.Image.Image':
            color = GriffinPalette.GROUP_11
        else:
            color = GriffinPalette.GROUP_12
...
class BaseTableView(QTableView, GriffinWidgetMixin):
    ...
    def __init__(self, parent):
        ...
        # (the default behavior for editing since Griffin was created; now they
    ...
    def mousePressEvent(self, event):
        ...
        if index_clicked.isValid():
            if (
                ...
                # Griffin 5 for users that are accustomed to do double clicks.
    ...
    def showEvent(self, event):
        ...
        # https://github.com/griffin-ide/griffin/pull/20933#issuecomment-1585474443
    ...
    def copy(self):
        ...
        for idx in self.selectedIndexes():
            ...
            # Fixes griffin-ide/griffin#12913
            try:
...
class CollectionsEditorTableView(BaseTableView):
    ...
    def oedit(self, key):
        """Edit item"""
        data = self.source_model.get_data()
        from griffin.plugins.variableexplorer.widgets.objecteditor import (
            oedit)
        oedit(data[key])
...
class CollectionsEditorWidget(QWidget, GriffinWidgetMixin):
    """Dictionary Editor Widget"""
    # Dummy conf section to avoid a warning from GriffinConfigurationObserver
    CONF_SECTION = ""
...
class CollectionsEditor(BaseDialog):
    ...
    def setup(self, data, title='', readonly=False, remote=False,
        ...
        # this would change the type after saving; cf. griffin-ide/griffin#6936.
...
class CollectionsCustomSortFilterProxy(CustomSortFilterProxy):
    ...
    def lessThan(self, left, right):
        ...
        try:
            ...
            # variable explorer. For more info see griffin-ide/griffin#14527
            return True
...
def get_test_data():
    ...
    return {'object': foobar,
            ...
            # Test for griffin-ide/griffin#3518.
...
def remote_editor_test():
    ...
    from griffin_kernels.utils.nsview import (make_remote_view,
                                             REMOTE_SETTINGS)
