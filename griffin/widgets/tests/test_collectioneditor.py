...
from griffin.widgets.collectionseditor import (
...
from griffin.plugins.variableexplorer.widgets.tests.test_dataframeeditor import (
    generate_pandas_indexes)
from griffin_kernels.utils.nsview import get_size
...
def test_collectionsmodel_with_index():
    # Regression test for griffin-ide/griffin#3380,
    # modified for griffin-ide/griffin#3758.
...
def test_shows_dataframeeditor_when_editing_index(monkeypatch):
    for __, rng in generate_pandas_indexes().items():
        ...
        attr_to_patch_dfedit = ('griffin.plugins.variableexplorer.widgets.' +
...
def test_collectioneditor_refresh_after_edit(result):
    ...
    with patch('griffin.widgets.collectionseditor.QMessageBox.question',
...
def test_collectioneditor_refresh_when_variable_deleted(qtbot):
    ...
    editor.setup(lst)
    with patch('griffin.widgets.collectionseditor.QMessageBox'
...
def test_edit_datetime(monkeypatch):
    ...
    Regression test for griffin-ide/griffin#13557 and griffin-ide/griffin#8329
    """
...
def test_edit_mutable_and_immutable_types(monkeypatch):
    ...
    Regression test for griffin-ide/griffin#5991.
    """
    MockQLineEdit = Mock()
    attr_to_patch_qlineedit = ('griffin.plugins.variableexplorer.widgets.' +
                               'collectionsdelegate.QLineEdit')
    monkeypatch.setattr(attr_to_patch_qlineedit, MockQLineEdit)

    MockTextEditor = Mock()
    attr_to_patch_textedit = ('griffin.plugins.variableexplorer.widgets.' +
                              'collectionsdelegate.TextEditor')
    monkeypatch.setattr(attr_to_patch_textedit, MockTextEditor)

    MockQDateTimeEdit = Mock()
    attr_to_patch_qdatetimeedit = ('griffin.plugins.variableexplorer.widgets.' +
    ...
    attr_to_patch_coledit = ('griffin.widgets.' +
...
def test_notimplementederror_multiindex():
    ...
    Regression test for griffin-ide/griffin#6284.
    """
...
def test_editor_parent_set(monkeypatch):
    """
    Test that editors have their parent set so they close with Griffin.

    Regression test for griffin-ide/griffin#5696.
    """
    # Mocking and setup
    test_parent = QWidget()
    ...
    attr_to_patch_arredit = ('griffin.plugins.variableexplorer.widgets.' +
    ...
    attr_to_patch_objectexplorer = ('griffin.plugins.variableexplorer.widgets.'
...
def test_xml_dom_element_view():
    ...
    Regression test for griffin-ide/griffin#5642.
    """
...
def test_pandas_dateoffset_view():
    ...
    Regression test for griffin-ide/griffin#6729.
    """
...
def test_set_nonsettable_objects(nonsettable_objects_data):
    ...
    Unit regression test for issues griffin-ide/griffin#6727 and
    griffin-ide/griffin#6728.
    """
...
def test_edit_nonsettable_objects(qtbot, nonsettable_objects_data):
    ...
    Integration regression test for issues griffin-ide/griffin#6727 and
    griffin-ide/griffin#6728.
    """
    for test_obj, expected_obj, keys in nonsettable_objects_data:
        col_editor = CollectionsEditor(None)
...
def test_collectionseditor_with_class_having_buggy_copy(qtbot):
    ...
    readonly; cf. griffin-ide/griffin#6936.
    """
...
def test_dicts_with_mixed_types_as_key(qtbot):
    ...
    This is a regression for griffin-ide/griffin#13481.
    """
