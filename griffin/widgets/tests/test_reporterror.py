...
# Local imports
from griffin import __project_url__
from griffin.widgets.reporterror import (DESC_MIN_CHARS, TITLE_MIN_CHARS,
                                        GriffinErrorDialog)
...
def error_dialog(qtbot):
    ...
    widget = GriffinErrorDialog(None)
...
def test_report_issue_url(monkeypatch):
    ...
    attr_to_patch = ('griffin.widgets.reporterror.QDesktopServices')
    ...
    GriffinErrorDialog.open_web_report(body=body, title=None)
    ...
    target_url = QUrl(target_url_base + '?body=' + body
                      + "&title=" + title)
    GriffinErrorDialog.open_web_report(body=body, title=title)
