...
from griffin.utils.stringmatching import get_search_regex
...
class HTMLDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if index.row() == self._hovered_row:
            painter.fillRect(
                options.rect, QColor(GriffinPalette.COLOR_BACKGROUND_3)
            )
...
class IconLineEdit(QLineEdit):
    ...
    def paintEvent(self, event):
        ...
        # See PR griffin-ide/griffin#20005 for context.
        if self.elide_text and not self._focus_in:
            ...
            text = fm.elidedText(self.text(), self.ellipsis_place,
                                 text_rect.width())
            painter.setPen(QColor(GriffinPalette.COLOR_TEXT_1))
        ...
        # Small hack to guarantee correct padding on Griffin start
...
class FinderLineEdit(ClearLineEdit):
    ...
    def __init__(self, parent, regex_base=None, key_filter_dict=None):
        ...
        self._combobox = GriffinComboBox(self)
...
class FinderWidget(QWidget):
    ...
    def __init__(self, parent, regex_base=None, key_filter_dict=None,
        ...
        # Parent is assumed to be a griffin widget
...
class PaneEmptyWidget(QFrame, SvgToScaledPixmap, GriffinFontsMixin):
    ...
    def __init__(
        ...
        interface_font_size = self.get_font(
            GriffinFontType.Interface).pointSize()
        ...
        if description is not None:
            ...
            description_label_qss.QLabel.setValues(
                ...
                backgroundColor=GriffinPalette.COLOR_OCCURRENCE_3,
                border="0px",
                padding="20px",
            )
    ...
    def _set_delegate(self):
        ...
        class HoverRowDelegate(QItemDelegate):
            ...
            def paint(self, painter, option, index):
                ...
                if index.row() == self._hovered_row:
                    painter.fillRect(
                        option.rect, QColor(GriffinPalette.COLOR_BACKGROUND_3)
                    )
...
def test_msgcheckbox():
    ...
    box = MessageCheckBox()
    box.setWindowTitle(_("Griffin updates"))
