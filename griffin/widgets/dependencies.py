...
"""Module checking Griffin runtime dependencies"""
...
from griffin.dependencies import OPTIONAL, PLUGIN
...
class DependenciesTreeWidget(QTreeWidget):
    ...
    def update_dependencies(self, dependencies):
        ...
        # Griffin plugins
        griffin_plugins = QTreeWidgetItem([_("Griffin plugins")])
        griffin_plugins.setFont(0, font)

        self.addTopLevelItems([mandatory_item, optional_item, griffin_plugins])
        ...
        for dependency in sorted(dependencies,
            ...
            if dependency.check():
                ...
                item.setBackground(2, QColor(GriffinPalette.COLOR_WARN_1))

                # Fix foreground color in the light theme
                if not is_dark_interface():
                    item.setForeground(
                        2, QColor(GriffinPalette.COLOR_BACKGROUND_1)
                    )
            else:
                item.setIcon(0, ima.icon('dependency_error'))
                item.setBackground(2, QColor(GriffinPalette.COLOR_ERROR_1))
...
class DependenciesDialog(QDialog):
    ...
    def __init__(self, parent):
        ...
        # Widgets
        note1 = _(
            "Optional modules are not required to run Griffin but enhance "
            "its functions."
        )

        note2 = _(
            "New dependencies or changed ones will be correctly detected only "
            "after Griffin is restarted."
        )
        ...
        ok_btn = GriffinDialogButtonBox(QDialogButtonBox.Ok)

        # Widget setup
        self.setWindowTitle(
            _("Dependencies for Griffin {}").format(__version__)
        )
        self.setModal(False)
    ...
    def copy_to_clipboard(self):
        from griffin.dependencies import status
