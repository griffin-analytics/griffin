...
from griffin.widgets.helperwidgets import HoverRowsTableView, HTMLDelegate
...
class ElementsModel(QAbstractTableModel, GriffinFontsMixin):
    ...
    def __init__(
        ...
        # Text styles
        text_color = GriffinPalette.COLOR_TEXT_1
        title_font_size = self.get_font(
            GriffinFontType.Interface, font_size_delta=1).pointSize()

        self.title_style = f'color:{text_color}; font-size:{title_font_size}pt'
        self.additional_info_style = f'color:{GriffinPalette.COLOR_TEXT_4}'
...
class ElementsTable(HoverRowsTableView):
        ...
        if row != self._current_row:
            ...
            if self._with_widgets:
                ...
                new_row_widget.setStyleSheet(
                    f"background-color: {GriffinPalette.COLOR_BACKGROUND_3}"
                )
    ...
    def _set_stylesheet(self, leave=False):
        ...
        bgcolor = GriffinPalette.COLOR_BACKGROUND_1 if leave else "transparent"
