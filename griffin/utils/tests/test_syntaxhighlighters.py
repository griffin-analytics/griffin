...
from griffin.utils.syntaxhighlighters import HtmlSH, PythonSH, MarkdownSH
...
def test_HtmlSH_basic():
    ...
    sh = HtmlSH(doc, color_scheme='Griffin')
...
def test_PythonSH_UTF16_number():
    ...
    txt = '𨭎𨭎𨭎𨭎 = 100000000'
    doc = QTextDocument(txt)
    sh = PythonSH(doc, color_scheme='Griffin')
    sh.rehighlightBlock(doc.firstBlock())
...
def test_Markdown_basic():
    ...
    doc = QTextDocument(txt)
    sh = MarkdownSH(doc, color_scheme='Griffin')
    sh.rehighlightBlock(doc.firstBlock())
