...
# Local imports
from griffin.py3compat import (is_string, to_text_string, is_binary_string,
                              is_text_string)
from griffin.utils.external.binaryornot.check import is_binary
...
def write(text, filename, encoding='utf-8', mode='wb'):
    ...
    if 'a' in mode:
        ...
        try:
            ...
            # See #griffin-ide/griffin#14112.
            original_mode = 0o666 & ~umask
            creation = time.time()
        try:
            # fixes issues with scripts in Dropbox leaving
            # temporary files in the folder, see griffin-ide/griffin#13041
            tempfolder = None
