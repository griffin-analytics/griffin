...
from griffin.utils.programs import run_shell_command
...
def _get_user_env_script():
    ...
    shell in a subprocess causes Griffin to hang on Linux. Using a shell script
    resolves the issue. Note that -i must be in the sha-bang line; if -i and
    -l are swapped, Griffin will hang.
    """
    script_text = None
...
def get_user_environment_variables():
    ...
    if os.name == 'nt':
        ...
        # Detect if the Griffin process was launched from a system terminal.
        # This is None if that was not the case.
        launched_from_terminal = psutil.Process(os.getpid()).terminal()

        # We only need to do this if Griffin was **not** launched from a
        # terminal. Otherwise, it'll inherit the env vars present in it.
        # Fixes griffin-ide/griffin#22415
        if not launched_from_terminal or running_under_pytest():
            try:
                ...
                # Use timeout to fix griffin-ide/griffin#21172
...
def amend_user_shell_init(text="", restore=False):
    ...
    m1 = "# <<<< Griffin Environment <<<<"
    m2 = "# >>>> Griffin Environment >>>>"
    if restore:
        ...
        if os.name == 'nt':
            ...
            QMessageBox.warning(
                ...
                _("If you accept changes, "
                  ...
                  "tion (simply restart Griffin if you have executed it "
