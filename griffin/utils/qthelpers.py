...
from griffin.config.base import is_conda_based_app
from griffin.config.manager import CONF
from griffin.py3compat import is_text_string, to_text_string
...
from griffin.utils.registries import ACTION_REGISTRY, TOOLBUTTON_REGISTRY
from griffin.widgets.waitingspinner import QWaitingSpinner
...
def qapplication(translate=True, test_time=3):
    ...
    if app is None:
        ...
        app = GriffinApplication(['Griffin', '--no-sandbox'])

        # Set application name for KDE. See griffin-ide/griffin#2207.
...
def create_waitspinner(size=32, n=11, parent=None, name=None):
    ...
    spinner.setColor(GriffinPalette.COLOR_TEXT_1)
...
def create_action(parent, text, shortcut=None, icon=None, tip=None,
    ...
    """Create a QAction"""
    action = GriffinAction(text, parent, action_id=id_)
    if triggered is not None:
    ...
    # ones).
    # Fixes griffin-ide/griffin#15659.
    if sys.platform == 'darwin':
...
def add_configuration_update(action):
    """Add on_configuration_change to a GriffinAction that depends on CONF."""
...
def create_module_bookmark_actions(parent, bookmarks):
    ...
    for key, url, title in bookmarks:
        # Create actions for scientific distros only if Griffin is installed
        # under them
        create_act = True
        if key == 'winpython':
...
class GriffinAction(QAction):
    """Griffin QAction class wrapper to handle cross platform patches."""

    def __init__(self, *args, action_id=None, **kwargs):
        """Griffin QAction class wrapper to handle cross platform patches."""
        super(GriffinAction, self).__init__(*args, **kwargs)
        self.action_id = action_id
        if sys.platform == "darwin":
            self.setIconVisibleInMenu(False)

    def __str__(self):
        return "GriffinAction('{0}')".format(self.text())
...
class GriffinApplication(QApplication, GriffinConfigurationAccessor,
                        GriffinFontsMixin):
    """Subclass with several adjustments for Griffin."""
    ...
    def __init__(self, *args):
        ...
        # This is filled at startup in griffin.app.utils.create_window
    ...
    def _set_monospace_interface_font(self, app_font):
        ...
        plain_font = self.get_font(GriffinFontType.Monospace)
        ...
        while (
            ...
            # Fixes griffin-ide/griffin#22661
            and attempts < 6
        ):
