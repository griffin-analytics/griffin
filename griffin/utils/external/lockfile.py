...
# Copyright (c) 2012- Griffin Project Contributors
#
...
Changes by the Griffin Team to the original module:
  * Rewrite kill Windows function to make it more reliable.
  * Detect if the process that owns the lock is an Griffin one.
...
if not os.name == 'nt':
    ...
    def symlink(value, filename):    #analysis:ignore
        ...
        try:
            ...
            # have permissions to write in ~/.griffin
            ...
            try:
                ...
                if e.errno == errno.EEXIST:
                    ...
                    try:
                        if kill is not None:
                            kill(int(pid), 0)
                        if not is_griffin_process(int(pid)):
