...
# and failure to read out buffers will most likely lock up Griffin.
...
# Local imports
from griffin.config.base import get_debug_level, STDERR
...
def read_packet(sock, timeout=None):
    ...
    if data is not None:
        try:
            ...
            # Catch all exceptions to avoid locking griffin
            if DEBUG_EDITOR:
...
# griffin-ide/griffin#857.
COMMUNICATE_LOCK = threading.Lock()

# * Old com implementation *
# See solution (1) in griffin-ide/griffin#434, comment 13:
...
# See solution (2) in griffin-ide/griffin#434, comment 13:
...
if __name__ == '__main__':
    if not os.name == 'nt':
        ...
        print("-- Testing standard Python socket interface --")  # griffin: test-skip
        ...
        print('..got "%s" from %s' % (accsock.recv(4096), addr))  # griffin: test-skip
        ...
        print("-- Testing BSD socket write_packet/read_packet --")  # griffin: test-skip

        write_packet(client, "a tiny piece of data")
        print('..got "%s" from read_packet()' % (read_packet(accsock)))  # griffin: test-skip

        client.close()
        server.close()

        print("-- Done.")  # griffin: test-skip
