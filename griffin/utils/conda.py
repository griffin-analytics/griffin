...
# Local imports
from griffin.utils.programs import find_program, run_program, run_shell_command
...
def find_conda(pyexec=None):
    ...
    # First try Griffin's conda executable
...
def get_list_conda_envs():
    ...
    for env in out['envs']:
        ...
        if (
            ...
            or (is_conda_based_app(pyexec=path) and name != "griffin-runtime")
        ):
            continue
...
def get_griffin_conda_channel():
    """Get the conda channel from which Griffin was installed."""
    conda = find_conda()
    ...
    cmdstr = ' '.join([conda, 'list', 'griffin', '--json', '--prefix', env])
    ...
    # Fixes griffin-ide/griffin#22054
    channel, channel_url = None, None

    for package_info in out:
        if package_info["name"] == 'griffin':
