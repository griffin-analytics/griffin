...
# Local imports
from griffin.config.base import _, running_under_pytest, get_home_dir
...
def get_temp_dir(suffix=None):
    """
    Return temporary Griffin directory, checking previously that it exists.
    """
    to_join = [tempfile.gettempdir()]

    if os.name == 'nt':
        to_join.append('griffin')
    else:
        username = encoding.to_unicode_from_fs(getuser())
        to_join.append('griffin-' + username)
...
def is_program_installed(basename, extra_paths=[]):
    ...
    launch Griffin. Additionally, extra_paths are searched.
...
def run_python_script_in_terminal(fname, wdir, args, interact, debug,
    ...
    if os.name == 'nt':
        ...
        f = tempfile.NamedTemporaryFile('wt', prefix='run_griffin_',
    ...
    if interpreter:
        cmd = dedent("""
            try:
                ...
                print('No Module')  # griffin: test-skip
            print(
...
def is_module_installed(module_name, version=None, interpreter=None,
    ...
    if interpreter is not None:
        ...
        # interpreter is None, just get module version in Griffin environment
        try:
...
def is_griffin_process(pid):
    """
    Test whether given PID belongs to a Griffin process.
    ...
    try:
        ...
        names = set(['griffin', 'griffin3', 'griffin.exe', 'griffin3.exe',
                     'bootstrap.py', 'griffin-script.py', 'griffin-script.pyw',
                     'Griffin.launch.pyw'])
