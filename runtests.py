...
"""
Script for running Griffin tests programmatically.
"""
...
def run_pytest(run_slow=False, extra_args=None, remoteclient=False):
    ...
    if CI:
        # Show coverage
        pytest_args += ['--cov=griffin', '--no-cov-on-fail', '--cov-report=xml']
    ...
    if remoteclient:
        pytest_args += ['--container-scope=class',
                        './griffin/plugins/remoteclient']
        os.environ["GRIFFIN_TEST_REMOTE_CLIENT"] = "true"
    else:
        pytest_args += ['--ignore=./griffin/plugins/remoteclient']
...
def main():
    """Parse args then run the pytest suite for Griffin."""
    test_parser = argparse.ArgumentParser(
        usage=('python runtests.py'
               '[-h] [--run-slow] [--remote-client] [pytest_args]'),
        description="Helper script to run Griffin's test suite")
