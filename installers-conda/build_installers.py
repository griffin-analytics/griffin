...
"""
Create Griffin installers using `constructor`.
...
p.add_argument(
    ...
    help="Print Griffin version and exit."
)
args = p.parse_args()
...
SPYVER = parse(re.split('([<>=]+)[ ]*', rt_specs['griffin'])[-1])
...
def _create_conda_lock(env_type='base'):
    ...
    definitions = {
        "channels": [
            "conda-forge",
            "conda-forge/label/griffin_rc",
            "conda-forge/label/griffin_dev",
            "conda-forge/label/griffin_kernels_rc",
            # TODO: Add this channel when we create it
            # "conda-forge/label/griffin_kernels_dev",
            CONDA_BLD_PATH,
        ],
...
def _generate_background_images(installer_type):
    ...
    logo_path = SPYREPO / "img_src" / "griffin.png"
...
def _get_condarc():
    contents = dedent(
        ...
        env_prompt: '[griffin]({default_env}) '  #! final
...
def _definitions():
    ...
    definitions = {
        "name": APP,
        "company": "Griffin-IDE",
        "reverse_domain_identifier": "org.griffin-ide.Griffin",
        "version": str(SPYVER),
    ...
    if WINDOWS:
        definitions.update(
            {
                "uninstall_name": f"Griffin {SPYVER.major}",
                "welcome_image": str(WELCOME_IMG_WIN),
                "header_image": str(HEADER_IMG_WIN),
                "icon_image": str(SPYREPO / "img_src" / "griffin.ico"),
                "default_prefix": os.path.join(
