...
class GriffinCondaPkg(BuildCondaPkg):
    name = "griffin"
    norm = False
    source = os.environ.get('GRIFFIN_SOURCE', HERE.parent)
    feedstock = "https://github.com/conda-forge/griffin-feedstock"
    feedstock_branch = "main"
    shallow_ver = "v5.3.2"

    def _patch_source(self):
        self.logger.info("Patching Griffin source...")

        file = self._bld_src / "griffin/__init__.py"
    ...
    def patch_recipe(self):
        # Get current Griffin requirements
        griffin_base_reqs = ['python']
        griffin_base_reqs += yaml.load(
            REQ_MAIN.read_text())['dependencies']
        if os.name == 'nt':
            win_requirements =  yaml.load(
                REQ_WINDOWS.read_text())['dependencies']
            griffin_base_reqs += win_requirements
            griffin_base_reqs.append('ptyprocess >=0.5')
            ...
            griffin_base_reqs += mac_requirements
            griffin_base_reqs.append('__osx')
        else:
            linux_requirements = yaml.load(
                REQ_LINUX.read_text())['dependencies']
            griffin_base_reqs += linux_requirements
            griffin_base_reqs.append('__linux')

        griffin_reqs = [f"griffin-base =={self.version}"]
        for req in griffin_base_reqs.copy():
            if req.startswith(('pyqt ', 'pyqtwebengine ', 'qtconsole ')):
                griffin_reqs.append(req)
                griffin_base_reqs.remove(req)

            if req.startswith('qtconsole '):
                griffin_base_reqs.append(
                    req.replace('qtconsole', 'qtconsole-base')
                )

        self.recipe_clobber.update({
            "requirements": {"run": griffin_base_reqs},
            ...
            "outputs": [
                ...
                },
                {
                    "name": "griffin",
                    "build": {"noarch": "python"},
                    "requirements": {"run": griffin_reqs},
                    "test": {
                        "requires": ["pip"],
                        "commands": ["griffin -h", "python -m pip check"],
                        "imports": ["griffin"]
                    }
                }
            ]
...
class GriffinKernelsCondaPkg(BuildCondaPkg):
    name = "griffin-kernels"
    source = os.environ.get('GRIFFIN_KERNELS_SOURCE')
    feedstock = "https://github.com/conda-forge/griffin-kernels-feedstock"
    feedstock_branch = "rc"
    shallow_ver = "v2.3.1"


PKGS = {
    GriffinCondaPkg.name: GriffinCondaPkg,
    PylspCondaPkg.name: PylspCondaPkg,
    QtconsoleCondaPkg.name: QtconsoleCondaPkg,
    GriffinKernelsCondaPkg.name: GriffinKernelsCondaPkg
}
...
if __name__ == "__main__":
    p = ArgumentParser(
        description=dedent(
            ...
            This module builds conda packages for Griffin and external-deps for
            ...
            provided for each package:
                ...
                QdarkstyleCondaPkg
                QtconsoleCondaPkg
                GriffinKernelsCondaPkg

            Griffin will be packaged from this repository (in its checked-out
            state). qtconsole, griffin-kernels, and python-lsp-server will be
